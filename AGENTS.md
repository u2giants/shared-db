...
1. **Up to three unrelated migrations may be authored at once. Preview, merges, and production
   ...
   **Do not open a migration file first.** Acquire an author lane, an exact object claim, and a
   ...
   - Probe reviewer process/session updates and a non-empty output stream before waiting. Replace
    ...
    not failed on the exact head, then fail closed with the exact blocker. The active rotation is
