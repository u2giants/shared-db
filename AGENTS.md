...
1. **Up to eight unrelated migrations may hold active-author capacity at once. Preview, merges,
   ...
   **Do not open a migration file first.** Acquire an author lane, an exact object claim, and a
   ...
   - Probe reviewer process/session updates and a non-empty output stream before waiting. Replace
    ...
    Grok 4.6, GLM 5.3, Kimi K3, Muse Spark 1.2 Contributor, Codex GPT-5.6 Sol, and
    Gemini 3.8 Flash High, minus the live orchestrator's own engine. Gemini re-entered
    on 2026-09-06 (PR #2438) after a live re-qualification. DeepSeek is inactive: it was
    RETIRED on 2026-09-01 (issue #2078) and is not drawable.
