...
The guarded row-application work is planned in [`plan_historical_mg_reclassification_apply.md`](plan_historical_mg_reclassification_apply.md). Read its STATUS table first, then §9.1, which records the completed 2026-09-02 read-only Phase 0 re-run and its live counts. It permits no preview or production write without a new explicit authorization, applies only complete live-qualified triplets in its first batch, keeps private artifacts out of this public repo, and leaves the May 14 cutoff in place until its exhaustive live-population gate passes.
