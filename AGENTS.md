...
- **Cutting issue lead time (guard truth, false-alarm corpus, blocker ledger):** [`plan_orchestrator_throughput_guard_truth.md`](plan_orchestrator_throughput_guard_truth.md). Read its STATUS table first — do not re-derive its analysis or re-plan its steps. Repository-maintenance work that authorizes **no** database change; do not route it to the structure/schema orchestrator. It fixes the class of blocker where a guard judges SQL text instead of the live catalog and reports "missing" for an object production already has.
...
| **6.15** | OWNER RULING — the property lists: TWO source kinds plus the reconciled layer the apps use (amended 2026-08-27), and `core.property` (Universe A) is DELETED and DISPOSABLE (Albert Hazan, 2026-08-19 / 2026-08-27) | [read](docs/owner-rulings.md#615-owner-ruling-there-are-exactly-two-kinds-of-property-list-and-coreproperty-universe-a-is-to-be-deleted-albert-hazan-2026-08-19) |
