...
import { buildInlineUpdate, INLINE_EDIT_REASON, saveInlineRow } from './inline-edit'
...
describe('inline table editing', () => {
  it('maps editable grid columns to the existing guarded update contract', () => {
    expect(buildInlineUpdate(row, 'name_display', 'Acme North')).toEqual({
      expectedUpdatedAt: row.updated_at,
      reason: INLINE_EDIT_REASON,
      displayName: 'Acme North',
    })
  ...
  it('uses each saved row as the concurrency token for the next cell', async () => {
    const update = vi.fn()
      .mockResolvedValueOnce({ success: true, row: { ...row, display_name: 'Acme North', updated_at: 'token-2' } })
      .mockResolvedValueOnce({ success: true, row: { ...row, display_name: 'Acme North', status: 'inactive', updated_at: 'token-3' } })

    const saved = await saveInlineRow('customer', row, {
      name_display: 'Acme North',
      status: 'inactive',
    }, update)
