...
| # | Step | State | Evidence |
|---|---|---|---|
| 0 | Apply the merged-but-unapplied `20260802170000` to production | ⬜ open | — |
| 1 | Inventory + freeze: `plm.curation_lock` design agreed | ⬜ open | — |
...
**A fresh session starts at Step 0.** Step 0 is independent of the rest and is the single
highest-value action in this document: it closes a rule violation that is live in production
right now.
...
- Applying the already-merged corrective migration `20260802170000` to production (Step 0).
...
#### Step 0. Apply `20260802170000` to production
...
- **Constraints it must carry:**
  - `resolution_status` domain CHECK matching the existing one:
    `IN ('unresolved','matched','ambiguous','no_match','rejected')`.
...
- **What:** a `BEFORE UPDATE` trigger function, installed on every `plm` landing table that
  carries resolution columns, that raises rather than allowing a write to those columns from a
  sync path.
- **The design question you must answer explicitly in the migration comment:** how does the
  trigger tell a human's curation from a loader's write? Two workable mechanisms, pick one and
  justify it:
  - **(a) Session setting.** Curation paths set a flag (`set local plm.curating = 'on'`); the
    trigger refuses the write unless it is set. Loaders never set it, so a new loader is
    refused by default — which is the correct failure direction.
  - **(b) Role-based.** The trigger checks `current_user` / `session_user` against an allowed
    curator role. Cleaner if the roles already separate loader from human; verify with
    `select rolname from pg_roles;` before choosing this.
  Recommendation: **(a)**, because it does not depend on how any future loader authenticates
  and it fails closed for anything that has not opted in.
...
- **Behaviour when done:** it is structurally impossible for a sync, import, backfill script,
  or future loader nobody has written yet to overwrite a curated decision. That is the goal in
  §1, restated as a database constraint.
