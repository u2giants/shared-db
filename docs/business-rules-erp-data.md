...
## Appendix: notes for the session that builds the historical load

Not business rules — practical advice from the session that probed the endpoints, kept here so it
travels with the meaning rather than only with the shape doc.

**Land the data raw first, model it second.** Store one row per API row exactly as returned (a
`jsonb` payload plus a few extracted key columns), stamped with the window it came from and when it
was fetched. Every question asked of this data so far turned into "re-probe the API and hope the
sample is representative". A raw landing table turns each one into a SQL query over everything.
Shape the curated tables **after** the full history is in and can be inspected whole.

**Record the fetch, not just the rows.** A window that returned zero rows and a window that was
never fetched must be distinguishable, or a gap in the history looks like a quiet period. Keep a
per-window log: endpoint, from, to, HTTP status, row count, duration, fetched-at. Resume from it.

**Make it resumable and idempotent from the start.** ~740 requests will be interrupted at least
once. Re-running a window must replace that window's rows, not append them.

**Do not reshape or clean on the way in.** Keep `1900-01-01`, keep `salesOrderNo = 0`, keep the
always-zero fields. Normalising during ingest destroys the evidence needed to answer the open
questions, and two of this project's findings came from noticing values that a "helpful" loader
would have stripped.

**Read these before writing code**, in this order:
1. [`coldlion-history-endpoints-shape.md`](coldlion-history-endpoints-shape.md) — §2 (7-day cap and
   the malformed error), §3 (pacing), §4.3 (row identity), §5 (fields that lie).
2. This file — what the data *means*.
3. `HANDOFF.d/2026-08-14T2236Z-al8960ofc-claude-coldlion-history-endpoints.md` — what was tried,
   what failed, and the open unknowns.

**Traps already paid for, do not rediscover:**
- `page`/`size` are accepted and ignored. Chunk by date only.
- The over-wide-window refusal is HTTP 400 on the wire but says `500` in the body. Do not retry it.
- Row identity on `prodHistory` is `(prodOrderNo, prodLineSeq, prepackItemNo)`. Do not build the old
  quantity-comparison dedupe; `prodLineSeq` replaced it.
- `tools/coldlion-sync-common.mjs` has reusable API-key handling, but its `fetchPaged` is **wrong**
  for these endpoints because it assumes paging.
- The API key comes from 1Password via `op` and is never pasted anywhere.

**Two things worth deciding deliberately, not by default:**
- *How far back to go.* Not yet known; June 2019 returns data and no earlier boundary was searched.
  ColdLion has been asked directly, which is cheaper than scanning.
- *Whether the same window returns the same rows on different days.* Untested, and it decides
  whether the recurring sync can trust a date window as a stable unit. Now cheap to test.
