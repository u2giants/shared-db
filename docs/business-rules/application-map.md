...
| Product, Item, SKU, style number, source identifier, reusable design, controlled item description, prepack, or assortment | [`product-items-and-identifiers.md`](product-items-and-identifiers.md) |
| Whether an Item Master entry is a real product, a fee or charge code, a raw material or component, a store fixture, or a non-inventory placeholder | [`product-items-and-identifiers.md`](product-items-and-identifiers.md), section *Non-inventory items*, and [`erp-orders-and-source-meaning.md`](erp-orders-and-source-meaning.md) for how reliable the ERP flag is |
| Counting items with a missing Licensor or Property, "unmapped" or "unresolved" item populations, which exclusions to apply before counting, or reconciling two different answers to the same count | [`unmapped-licensor-population.md`](unmapped-licensor-population.md) |
