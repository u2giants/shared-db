...
from hierarchical_item_taxonomy import CUTOFF, ParsedDescription, build_associations, classify_product_type, load_product_dictionary, parse_description, usable_description
...
def run(source: Path, dictionary_path: Path, output: Path) -> dict:
    ...
    _, reverse = build_associations(data[~heldout])
    rows = []
    for _, row in data[heldout].iterrows():
        if row["Dictionary Status"] not in {"accepted", "alias"} or not row["Product Type"]:
            continue
        decision = classify_product_type(row["Product Type"], reverse, row["Construction Shape"], row["Treatment"])
        actual = "|".join(row[f"MG0{i}"].strip().upper() for i in range(1, decision["depth"] + 1))
        rows.append({"level": decision["depth"], "product": row["Product Type"], "construction": row["Construction Shape"], "proposed": decision["key"], "actual": actual, "correct": decision["depth"] > 0 and decision["key"] == actual})
    ...
    result = {"post_change_rows": len(data), "heldout_rows": int(heldout.sum()), "eligible_heldout_rows": len(detail), "by_level": by_level}
    result["errors"] = detail[~detail["correct"]].groupby(["level", "product", "construction", "proposed", "actual"]).size().reset_index(name="count").sort_values("count", ascending=False).head(100).to_dict("records")
