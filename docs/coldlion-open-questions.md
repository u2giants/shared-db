...
ones. This is the single register. **Last reviewed: 2026-08-31 (fourth pass — ColdLion added paging + inline label/warehouse/merch-group descriptions).**
...
| 2.12 | 🔴 **RE-OPENED 2026-08-31 with a quantity-multiplication finding — the invoiced half is a live data fault, not a curiosity.** Live scan of 2,502 order-history rows Jan–Aug 2026: **130 rows carry `salesOrderLineNo` = 0, and 66 of those also carry an invoice number**, across just five orders, all invoiced 2026-07-30 — so this is **current, not historical**. Every one shows the same shape: one item, one invoice, one order quantity, 12–14 rows all with line 0, and the invoiced quantity on each populated row equal to **orderQty × (number of populated rows)**. Order 7127866 / invoice 6016766 / item NHNQ601: orderQty 239, 14 rows, 7 populated, each showing 1,673 = 239 × 7; summing all rows gives 11,711, about **49× the true figure**. Same for 7127867 (203 → 1,421, ×7), 7127870 (134 → 938, ×7), 7127943 (259 → 1,554, ×6), 7127942 (149 → 894, ×6). **Loader consequence: `lineInvoiceQty` cannot be trusted from this feed until ColdLion confirms whether the multiplication is confined to these cases.** Quarantine any order whose rows are all line 0. Re-asked in [`coldlion-reply-draft-20260831.md`](coldlion-reply-draft-20260831.md) with the table and three questions. Original entry follows. **`salesOrderLineNo` = 0 on 103 rows (issue 7) — PARTIALLY answered 2026-08-28, still open.** JamieLynn: *"It looks like in most of these cases the items or orders were canceled. The invoiced orders are strange though. Tech team will look into it. Not sure what caused it."* Canceled items explain most of the 103. **The invoiced ones are unexplained and are with ColdLion's tech team.** Loader consequence: `salesOrderLineNo = 0` is not a usable line key — do not assume the new line key is populated on every row, and quarantine rows where it is 0 with an invoice number. Named examples already sent: orders 7114595, 7124128, 7126086. | JamieLynn | [`coldlion-19k-row-resample-20260827.md`](coldlion-19k-row-resample-20260827.md) | **Partially answered 2026-08-28; remainder with their tech team, no date given.** |
...
| 2.21 | **Issue 12 — four overlapping item lifecycle flags; which is authoritative?** Re-verified live 2026-08-28 on the full catalogue of 19,362 items (supersedes an earlier, stale 18,911-item count): `itemStatus` `A` on 6,676 and blank on 12,686; `active` `N` on 459; `itemDiscontinued` `Y` on 546; `itemAvailable` `N` on 11. We currently use `active`, which is a guess. Also ask whether the blank `itemStatus` on two-thirds of the catalogue means anything. ⚠️ Do not confuse this with the **merch-group** `active` flag, which IS settled (Y/N, ColdLion 2026-08-28) — different object. **Kept as a real ask: it decides what we treat as sellable.** | JamieLynn | [`coldlion-reply-draft-20260831.md`](coldlion-reply-draft-20260831.md) | **NOT SENT; in the 2026-08-31 draft, waiting on Albert to send.** |
