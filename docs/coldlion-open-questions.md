...
| 2.30 | **`/merchGroupDetails` returns 200 with zero rows under every query we can construct - found and asked 2026-09-03.** Unfiltered, `active=Y/N/empty`, `mgTypeCode=01` and `=06`, a real `mgCode`, company+division, and the full four-part key all return 0 rows, while `/merchGroupHeaders` returns 57 rows and defines ten live types (01 Type, 02 Sub-Type, 03 Sub-Sub-Type, 04 Size, 05 Licensor, 06 Property, 07 Style Guide, 08 Art Source, 09 Artist, 10 Demographic). The data provably exists: `/items` rows carry both a merch-group code and its description inline. **This partly undercuts the 2026-08-28 answer that told us to retrieve merch groups "via /merchGroupHeaders and /merchGroupDetails"** - half of that pair returns nothing. **Loader consequence: the code-to-description list can only be harvested item by item today.** Merch group codes drive our product taxonomy, so this matters more than its newness suggests. | JamieLynn | live probes 2026-09-03 | **SENT 2026-09-06 in the nine-section reply. Awaiting ColdLion's answer.** |
