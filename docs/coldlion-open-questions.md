...
| 2.21 | **Issue 12 — four overlapping item lifecycle flags; which is authoritative?** On 18,911 items: `itemStatus` `A` on 6,234 and blank on 12,677; `active` `N` on 408; `itemDiscontinued` `Y` on 546; `itemAvailable` `N` on 10. We currently use `active`, which is a guess. Also ask whether the blank `itemStatus` on two-thirds of the catalogue means anything. | JamieLynn | [`coldlion-reply-draft-20260828.md`](coldlion-reply-draft-20260828.md) | **NOT SENT. Drafted 2026-08-28, waiting on Albert to send.** |
