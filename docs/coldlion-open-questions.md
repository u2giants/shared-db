...
ones. This is the single register. **Last reviewed: 2026-08-28 (second pass).**
...
|---|---|---|---|---|
| 2.14 | **NEW, NOT YET SENT — which of the four documents did this row come from?** Their 2026-08-28 answer to issue 6 established that `orderHistory` assembles **Sales Order, Prepack Detail, Pick Ticket and Invoice**, and that the line number is re-assigned at pick and at invoice. The feed does not say which document a row came from, so we cannot tell a sales-order row from an invoice row, cannot pick one row per line, and cannot sum anything safely. **This is the single field that would make the feed usable.** Ask for a document-type or source-stage marker on every `orderHistory` row, and — if it exists — the pick-ticket and invoice numbers alongside it. | JamieLynn | [`coldlion-19k-row-resample-20260827.md`](coldlion-19k-row-resample-20260827.md), [`business-rules/erp-orders-and-source-meaning.md`](business-rules/erp-orders-and-source-meaning.md) | **NOT SENT. Drafted 2026-08-28, waiting on Albert to send.** |
| 2.15 | **The shipping and invoicing fields are still empty on every historical row.** `invoiceNoString`, `invoiceDateString`, `lineInvoiceQty`, `shipQty` and `shipAmount` return nothing on historical rows. Their 2026-08-28 answer to issue 2 — *"If it has an invoice number, unless we shipped short or partial Open / Unshipped would drop to zero"* — is a rule we **cannot apply**, because the invoice number it depends on is not in the feed. Either populate those fields on history, or tell us they never will be so we stop planning around them. | JamieLynn | [`coldlion-19k-row-resample-20260827.md`](coldlion-19k-row-resample-20260827.md) | **Sent 2026-08-27 18:34 EST as part of the item-1 correction; the 2026-08-28 reply did not address it. Re-ask with 2.14.** |
...
| 2.13 | **Issue 3 remainder — allowed-value lists and field descriptions in the spec.** `stageCode` was closed 2026-08-27; still open: `stageCode` is the only `enum` in the whole spec, and **no response field has a description** in any of the seven definitions. Narrowed ask sent: `mgTypeCode`, `divisionCode`, `active`, and the undocumented response fields `labelCode`, `warehouseCode`, `sizeCode`, `colorCode`, `dimCode`, `merchGroup01-06`. | JamieLynn | [`coldlion-19k-row-resample-20260827.md`](coldlion-19k-row-resample-20260827.md) | **Sent 2026-08-27 18:34 EST. No answer in the 2026-08-28 reply — still awaiting.** |
...
| **Issue 2 — do the new quantity formulas mean anything on historical rows?** | **"If it has an invoice number, unless we shipped short or partial, Open / Unshipped would drop to zero."** So a zero open/unshipped quantity on an invoiced row is a **true** zero, not an un-backfilled history. **Loader consequence: treat zero open/unshipped as real when an invoice number is present.** The remaining gap is unchanged and is not their doing — the feed still returns nothing in `invoiceNoString`, `lineInvoiceQty`, `shipQty`, `shipAmount` or `invoiceDateString` on historical rows, so **we cannot see the invoice number this rule depends on** and cannot apply the test row by row | JamieLynn, 2026-08-28 |
