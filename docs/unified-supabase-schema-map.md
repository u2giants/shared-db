...
|---|---|---|---|---|
| People and auth | PopDAM `profiles`, `user_roles`, `invitations`, `app_access`; PM/CRM `directus_users`; PLM `users`, `Roles`, `RolePermissions`, `UIElements`, `auth_token`, `quote_auth_token` | `app.profile`, `app.role`, `app.user_role`, `app.app_access` | shared | Supabase Auth should own identity. PLM and Directus users need cross-reference tables, not copied auth systems. **DesignFlow staff office + UI language live on `dflow.users`, not `app.profile`** (added 2026-08-10 by `20260810160000`, issue #690): `office_location text` (`'ningbo'`/`'nyc'`/NULL, CHECK `users_office_location_check`) and `preferred_language text` (`'en'`/`'zh-CN'`/NULL, CHECK `users_preferred_language_check`). Both nullable, no default, never backfilled — assignment happens through DesignFlow's Manage Users workflow from an authoritative roster. The app must NEVER infer office or language from email, name, IP, browser language, or time zone. Resolution rule: `coalesce(preferred_language, case when office_location = 'ningbo' then 'zh-CN' else 'en' end)`. Office values are stored lowercase to match the existing Sample Tracking vocabulary (`'ningbo'`/`'nyc'`); display strings are mapped in the UI layer. Contract test: `supabase/tests/dflow_user_office_language_contract.sql`. |
| Customers | PM/CRM `retailer`; PLM `customers`, `externalCustomer`; DAM `style_groups.customer`, `assets.customer`, `prod_order_headers_current.customer_*` | `core.customer`, `core.company_source_ref` | shared | One canonical customer hub holding potential + active customers. `is_potential = false` (active) only when an ERP/PLM source ref is present; everything else is potential. Preserve PLM customer ids, Directus ids, and DAM path customer strings as source refs. See `shared-database-vision.md` → "Customer vs. Company vs. Ingested Domain". Renamed from `core.company` via hard rename (no compat view). **Coldlion ERP `/customers` imported 2026-07-15** (`source_system='coldlion'`) via `plm.import_coldlion_customers()`; typed mirror in `plm.erp_customer`. Active-only promotion into canonical; `status` is app-owned and survives re-pulls (Coldlion's `active` flag is unreliable). `core.customer` 139 → 929. See `app-migration-notes/coldlion-customers-vendors-20260715.md`. |
...
| Product description phrase | PopDAM/Master Data SKU descriptions such as `Printed Glass Shadowbox`, `Coir Doormat`, `PE Rattan 2-Tier Wall Shelf` | `core.product_material` | shared | User-facing Product Type + Material picker phrase; separate from MG01 product type but can link to product type/subtype. |
| Shared packaging lookups | DAM Settings-managed packaging values; future PM/PIM, PLM, and DAM product/package pickers | `core.packaging_type` | shared | Additive lookup added by `20260709144500_core_packaging_type.sql`. It stores approved packaging type names, optional stable codes, active/inactive status, metadata, and timestamps. Browser writes are restricted by RLS to administrators plus `apinilla@popcre.com`; broad authenticated shared-role reads are allowed for app pickers. DAM exposes the editor at Settings -> Reference Data -> Packaging Types. |
| Factories and vendors | PM/CRM `factory`; PLM `Factory`, `vendor`, `vendorGroup`, `externalVendor`, `StandardizedVendor`; DAM `style_groups.factory/vendor` only if derived from ERP/path later | `core.factory`, `core.vendor_contact`, `core.factory_group` | shared | PLM has factory/vendor semantics; PM/CRM need the same rows for row scoping and production visibility. **Coldlion ERP `/vendors` imported 2026-07-15** (`source_system='coldlion'`) via `plm.import_coldlion_vendors()`; typed mirror in `plm.erp_vendor`; `core.factory` 6 → 529 rows. Active-only promotion; `status` is app-owned and survives re-pulls (see `app-migration-notes/coldlion-customers-vendors-20260715.md`). |
...
| Designs, art pieces, and design collections | PM `design`, `design_collection`; DAM `assets`, `style_groups`; PLM `artPiece`, `artPieceAttachment` | `pim.design`, `pim.design_collection`, `dam.asset`, `dam.style_group`, `plm.art_piece`, `plm.art_piece_item` | shared boundary | PM design records should link to DAM assets/style groups, not duplicate files. PLM art pieces are shared operational artwork records; `plm.art_piece_item` is the junction table linking one art piece to many SKUs/styles/items. |
...
| ERP enrichment in DAM | PopDAM `erp_items_current`, `erp_items_raw`, `erp_sync_runs`, `erp_enrichment_log`, `prod_order_headers_current`, `prod_order_headers_raw`, `prod_order_sync_runs` | `ingest.*` raw, `plm.*` current (item mirror + production orders); read through `api.*` views | staging/import | These are snapshots into PopDAM. **Active relocation in progress** — see [`fix_schema_for_api.md`](../fix_schema_for_api.md) (5-phase plan). **Phase 1 shipped 2026-07-15**: `api.plm_item_list` now serves `erp_items_current`, and `style_tracker_rows_with_bridge` reads through it — so the physical tables can move in later phases without breaking readers. Items are live (17.7k rows, fed Coldlion→dflow→Supabase, `source_system='designflow'`); prod-order tables are empty scaffolding to be built native. Long-term, items resolve into `plm.item` and production orders into `plm.production_order[_line]`. |
...
`dflow.users` is the authoritative DesignFlow **staff** user table (`designflow-backend` `GET /api/user/me` reads it for every non-vendor role; `role === 'vendor'` reads `dflow.vendor` instead). Since `20260810160000` it also carries `office_location` and `preferred_language` — see the People and auth row above for the allowed values and the language resolution rule.
