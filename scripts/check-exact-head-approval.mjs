...
import { REPO, REVIEW_ASSIGNMENT_REF_PREFIX, REVIEW_REPLACEMENT_REF_PREFIX, isApprovalFor, isVerdictFor } from './manage-migration-author-lanes.mjs'
...
export function evaluateExactHeadApproval({ pr, headSha, evidence = [], assignments = [] }) {
  ...
  const refusals = atHead.filter((row) => isVerdictFor(row,headSha) && !isApprovalFor(row,headSha))
  ...
  const approvals = evidence
    ...
    .filter((row) => isApprovalFor(row,headSha))
