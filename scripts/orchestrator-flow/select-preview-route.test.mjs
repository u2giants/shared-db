...
const classification=(impact='database-structure')=>{const files=[{path:'change.sql',sha256:'c'.repeat(64),impact,reason:`proved ${impact}`}],applicable_checks=['unit-tests'],required=!['documentation','reviewer-tooling','ci-workflow','read-only-test','application-only'].includes(impact);return{schema_version:1,decision:required?'DATABASE_PREVIEW_REQUIRED':'NO_DATABASE_PREVIEW',reason_code:required?`impact_${impact.replaceAll('-','_')}`:'proven_non_database_change',inspected_digest:sha256(canonicalJson({classifier_version:1,files,applicable_checks})),files,applicable_checks,invalidated_by}}
const baseClassification=classification()
const base={issue:1720,pr:1721,head_sha:'a'.repeat(40),bundle_id:'b'.repeat(64),database_preview:baseClassification,inspected_files:baseClassification.files.map(({path,sha256})=>({path,sha256})),versions:['20260828030000'],main_versions:['20260828010000'],preview_versions:['20260828010000'],claims:[],dependency_closure_complete:true,merged:false}
...
test('proven non-database work selects no preview without migration inputs',()=>{const database_preview=classification('application-only'),result=selectPreviewRoute({issue:1,pr:2,head_sha:'a'.repeat(40),bundle_id:'b'.repeat(64),database_preview,inspected_files:database_preview.files});assert.equal(result.route,'NO_DATABASE_PREVIEW');assert.deepEqual(result.context.applicable_checks,['unit-tests'])})
...
test('database evidence bundle produces an explicit preview-required classification bound to every evidence file',()=>{
  ...
  const bundle={identity,bundle_id:sha256(canonicalJson(identity))},result=databasePreviewRequiredFromEvidenceBundle(bundle)
