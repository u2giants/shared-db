...
const classified=(impact)=>{const files=[{path:'change',sha256:'c'.repeat(64),impact,reason:'exact impact evidence'}],applicable_checks=['unit-tests'],required=impact!=='application-only';return{schema_version:1,decision:required?'DATABASE_PREVIEW_REQUIRED':'NO_DATABASE_PREVIEW',reason_code:required?`impact_${impact.replaceAll('-','_')}`:'proven_non_database_change',inspected_digest:sha256(canonicalJson({classifier_version:1,files,applicable_checks})),files,applicable_checks,invalidated_by:['file-content-change','file-set-change','impact-evidence-change','applicable-check-change','classifier-version-change']}}
const databasePreview=classified('database-structure')
const preview={issue:1,pr:2,head_sha:'a'.repeat(40),bundle_id:'b'.repeat(64),database_preview:databasePreview,inspected_files:databasePreview.files,versions:['20260828030000'],main_versions:['20260828010000'],preview_versions:['20260828010000'],claims:[],dependency_closure_complete:true,merged:false}
