...
test('low quota with a near reset waits once, then continues', () => {
  ...
  const waits = []
  const result = checkQuota({ read: () => answers.shift(), wait: (ms) => waits.push(ms), now: () => NOW, env: {}, log: quiet })
  assert.equal(result.ok, true)
  assert.deepEqual(waits, [301000])
})

test('low quota with a far reset fails with a plain installation-quota message', () => {
  const result = checkQuota({ read: () => body(12, 40 * 60), wait: () => assert.fail('waited past 15 minutes'), now: () => NOW, env: {}, log: quiet })
...
test('still low after the wait, or unreadable, fails closed', () => {
  const stillLow = checkQuota({ read: () => body(3, 60), wait: () => {}, now: () => NOW, env: {}, log: quiet })
