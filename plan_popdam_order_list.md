...
| 6. Land shared-db, seed production, then deploy PopDAM | ⬜ open — **partly overtaken by events; schema half is DONE** | 2026-08-12 | The shared-db half of this step is finished: all four OrderList migrations are merged to `main` and live on production (see Step 1), so **nothing remains to promote**. What remains is (a) the **production data import**, which is not executable — `scripts/import-order-list-xlsx.py` refuses any write without `--preview` and states "This importer has no production mode" (line 1545); building the guarded production path is open issue **#852**, in flight — and (b) the PopDAM deploy, which per issue **#853** is **explicitly not approved to deploy**. Read `docs/verification/popdam-order-list-production-approval-2026-08-12/README.md` before any production import, applying the two corrections this table makes to it (Step 1a on the red run, Step 2 on the refuted "86/38" figure). **Two verified facts strengthen the approval case.** (i) **Preview and production are behaviourally identical**, compared 2026-08-12 by md5 of all three RPC bodies (`create_dam_order`, `update_dam_order`, `link_dam_order_line`), md5 of `pg_get_viewdef` for `api.dam_order_list`, `security_invoker = true` on both, all four NULLS-DISTINCT unique indexes checked at `pg_index.indnullsnotdistinct` level rather than by `constraintdef` text (which would hide exactly this kind of divergence), and all 12 RLS policies — **zero differences**. This is the strongest evidence the package has that a preview-proven import will behave the same in production. (ii) **Production holds zero OrderList rows**: all four `plm.production_order*` tables are empty and no `google_order_list` source refs exist, so approval gate 3 is satisfied. **Before the import runs, the RLS finding in OPEN QUESTIONS item 2 must be decided.** |
...
Two items that stood open on 2026-08-12 have since been resolved and moved into the STATUS table: the red B9 run is confirmed verifier-only (Step 1a) and the "86/38" figure is refuted (Step 2). What remains:
...
2. **Every authenticated account can read all four OrderList tables. Decide this BEFORE the production import, not after.** Verified 2026-08-12 on **both** databases: the policies `production_order_popdam_read`, `production_order_line_popdam_read`, `production_order_source_ref_read` and `production_order_line_source_ref_read` are all `USING (true)` for `authenticated`. Postgres OR-s permissive policies together, so these **fully subsume** the restrictive `plm_read` policy that was meant to narrow access — the narrow policy has no effect. This is harmless today only because the tables are empty (see Step 6), and it becomes a real exposure of customer, vendor and order data **the moment the historical import lands**. Treat it as a blocker on the production import: either the open-read posture is a deliberate decision matching the Master Data collaborative model and should be written down as such, or the policies need a corrective migration authored in shared-db first. It has not been decided either way.
