...
| 1. Add the shared database contract | ✅ complete — schema applied on BOTH preview and production | 2026-08-12 | **Four** migrations make up the contract, all on `origin/main` (tip `49fe2fe`): `20260810010000` (commit `523bda0`), `20260810060000` (`35b6494`), `20260810100000` (`59774a7`), and `20260810110000` (`4c98e52`, which sets `api.dam_order_list` to `security_invoker`). Earlier wording naming only three migrations and citing PR #663 was incomplete — #663 is a multi-issue integration merge, not the OrderList contract PR. **All four are live on preview `rjyboqwcdzcocqgmsyel` and production `qsllyeztdwjgirsysgai`**, verified 2026-08-12 by a read-only catalog check (`to_regclass` on the tables, `reloptions = {security_invoker=true}` on `api.dam_order_list`, and identical signatures for all three RPCs on both projects) — **not** by the migration ledgers, which disagree (preview 430 applied / max `20260812211000`; production 426 applied / max `20260812020000`; `20260810140000` and `20260810180000` are on production but absent from preview, so preview is not a superset). **Consequence: there is no OrderList migration left to promote.** The only outstanding production write is the data import. See OPEN QUESTIONS for the red B9 run. |
| 2. Rehearse and verify the database contract on preview | ✅ complete — proven behaviourally, not by a stored assertion artifact | 2026-08-12 | The prior wording claimed "86 object assertions and 38 behavior assertions" on preview. **That claim has no artifact anywhere under `docs/verification/` and carries no run ID or date** — it survives only as prose in `HANDOFF.d/2026-08-10T0030Z-al8960ofc-claude-orchestrator-nine-agent-fan-out.md` and `docs/verification/popdam-order-list-production-approval-2026-08-12/README.md`. **Do not rely on it.** The contract is instead proven by behaviour recorded in `docs/verification/popdam-order-list-preview-2026-08-12/README.md`: the preview import inserted **3,212 orders and 24,010 lines**, which is impossible unless `20260810060000` (the NULLS DISTINCT fix to the legacy source pair) is applied, since without it a second line per order cannot exist; a second identical run changed **0** business rows; and **all 10 balance checks PASS (overall BALANCED)**. |
...
| 4. Build the PopDAM OrderList page | ⬜ open — **blocked** | 2026-08-12 | Not started. No `/orders` route, page, or component exists in `u2giants/popdam3`. **Blocker: `plm.item` is empty**, so the page has nothing to link to: of the preview lines, **23,649 have `resolution = unique` but `item_id` NULL** (`docs/verification/popdam-order-list-preview-2026-08-12/README.md`) and `master_data_match_status = matched` is **0**. Populating `plm.item` and repointing the style-item bridge is owned by `fix_schema_for_api.md` Phase 4 and tracked in open issue **#853**. Building the grid before that would ship a screen whose Master Data column is empty on every row. |
| 5. Test the full workflow against preview | ⬜ open — blocked by Step 4 | 2026-08-12 | Cannot start: there is no page to test. Once Step 4 exists, this needs authenticated visual checks against preview `rjyboqwcdzcocqgmsyel` plus the import reconciliation already captured in Step 3. |
| 6. Land shared-db, seed production, then deploy PopDAM | ⬜ open — **partly overtaken by events; schema half is DONE** | 2026-08-12 | The shared-db half of this step is finished: all four OrderList migrations are merged to `main` and live on production (see Step 1), so **nothing remains to promote**. What remains is (a) the **production data import**, which is not executable — `scripts/import-order-list-xlsx.py` refuses any write without `--preview` and states "This importer has no production mode" (line 1545); building the guarded production path is open issue **#852**, in flight — and (b) the PopDAM deploy, which per issue **#853** is **explicitly not approved to deploy**. Read `docs/verification/popdam-order-list-production-approval-2026-08-12/README.md` before any production import; note that the same file repeats the unbacked assertion claim corrected in Step 2. |
| 7. Verify production and update durable docs | ⬜ open — blocked by Step 6 | 2026-08-12 | Cannot start. No production OrderList data exists and `https://dam.designflow.app/orders` is not built or deployed. |

**Fresh-session starting point:** Step 4 is the next build step, but it is **blocked** until `plm.item` is populated (issue #853 / `fix_schema_for_api.md` Phase 4). Do not repeat Steps 0–3. There is **no OrderList schema left to promote to production**; the only outstanding production write is the data import, and the importer has no production mode yet (issue #852). Before any production data import, read the exact approval package in [`docs/verification/popdam-order-list-production-approval-2026-08-12/README.md`](docs/verification/popdam-order-list-production-approval-2026-08-12/README.md), keeping in mind the corrections in Step 2 and in OPEN QUESTIONS below.

## OPEN QUESTIONS

These are recorded, not resolved. Do not treat any of them as settled, and do not delete a row until it has a dated artifact behind it.

1. **The source workbook lost five rows and nobody knows which five.** The approved Phase 0 baseline is **12,328** populated rows (SHA-256 `4958b4b7…`). A later read of the live Google sheet found **12,323**, and the currently accepted workbook (SHA-256 `68c9b03a0ec183e08b3a8f2344397e1bc4f61e73457849e7bf8c0cf7fb2409fe`) has **12,354**. The 12,328 → 12,354 move is explained: the owner re-approved a newer export, and Step 3's balance checks all reconcile against 12,354. **The 12,328 → 12,323 shrink is not explained.** Recorded as still unanswered in `HANDOFF.d/2026-08-11T2036Z-al8960ofc-claude-orchestrator-first-gated-production-apply.md` line 152. Someone edited the live sheet between exports; **which five rows disappeared, and whether they were real orders, has never been determined.** This matters because the historical import is a one-time seed.
2. **The "86 object assertions and 38 behavior assertions" rehearsal artifact cannot be found.** The number appears only as prose in a handoff and in the production approval README. There is no directory under `docs/verification/`, no run ID, and no date behind it. Either the artifact exists somewhere not yet searched, or the figure is a restatement that was never backed. **Until it is located, treat it as unverified** and rely on the behavioural evidence cited in Step 2 instead.
3. **Production apply run `31620553795` finished RED and that has not been independently confirmed as harmless.** Confirmed facts: the run is workflow "Shared Supabase Migrations", created 2026-08-12T17:01:40Z, conclusion **`failure`** (`https://github.com/u2giants/shared-db/actions/runs/31620553795`); PR **#843** "Fix post-apply ledger-aware catalog verification" merged at 2026-08-12T17:07:49Z, six minutes later. The **claim** — recorded in `docs/verification/popdam-order-list-production-approval-2026-08-12/README.md` — is that the database push succeeded and the run went red only because the post-apply verifier wrongly demanded already-applied companion migration `20260810180000`. **That claim is not verified in this document.** What independently supports it is the catalog check in Step 1 showing the OrderList objects genuinely present on production. What is still missing is a direct reading of the run's own logs proving the push step itself was green. A separate verification of this was in progress on 2026-08-12; **record its result here when it lands.**
