...
- **#2439 — blocked on owner hand-setting three values.** The alert relay is merged (popdam3 PR #126, merge `50c6818408a99a69f5e85029ba48b7270bba5876`) but not deployed. Railway needs `BULK_OPERATION_ALERT_WEBHOOK_URL`; Supabase needs `BULK_OPERATION_ALERT_SECRET` and `BREVO_API_KEY`. Owner authorized `hello@popcre.com` as the recipient. Live acceptance then needs a deliberate failed rebuild alert followed by a successful run.
