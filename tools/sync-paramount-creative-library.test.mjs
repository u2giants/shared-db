...
// pmt_property_capture_log.property_name were deprecated by migration 20260814170219;
...
test("the database boundary receives no name key: the serialized chunks omit both columns", () => {
  ...
  // migration 20260814170219 dropped the NOT NULL. If either key reappears here, a
...
// (20260814170219). These pin, OFFLINE, the invariants the database contract test
...
async function readDeprecationMigration() {
  ...
  return readFileSync(
    join(dirname(fileURLToPath(import.meta.url)), "..", "supabase", "migrations",
      "20260814170219_pmt_duplicate_name_columns_deprecated.sql"),
    "utf8"
  ).replace(/\r\n/g, "\n");
}
