...
Runtime preview-ledger truth comes from the existing read-only Management API path, not guesswork or a pinned preview literal. Read the live repository variable with `gh variable get PREVIEW_PROJECT_REF --repo u2giants/shared-db`, require `^[a-z]{20}$` exactly as the workflow does, refuse unset values, and refuse equality with `PROJECT_REFS.production`. Factor `APPLIED_VERSIONS_SQL` and `fetchAppliedVersions(projectRef)` into a read-only helper whose required project-ref parameter comes from that validated variable. Step 5 must never import, default to or fall back to `PROJECT_REFS.preview`; the existing drift command may keep its separately maintained literal for its own bounded target contract. The operator supplies the existing GitHub authentication and `SUPABASE_ACCESS_TOKEN` through protected environment/1Password procedures; the command quotes the validated preview project ref before the single constant SELECT and never writes. Missing authentication/token, invalid/production-equal ref, repository-variable read failure, HTTP failure or malformed rows is `UNVERIFIABLE` exit 2. Prior artifacts/events provide provenance and route evidence but never replace both fresh live-identity and ledger reads immediately before manual dispatch.
...
**Verification gate:** transcript fixtures reproduce #1720 waiting behind #1713, #1720 historical-route selection and #1646 dependency closure. Selector/reconcile tests prove the ref-safe `ready-id` changes with issue, head, bundle or route; wait creates no Actions/check/apply artifact; no marker reports readiness without mutation; a matching marker creates/readbacks one exact ready ref/event; two terminal writers produce one outcome ref; stale head/bundle/route supersedes; issue close/cancel/merge terminalizes an orphan from exact proof; repeated reconciliation emits no duplicate; and a later matching orchestrator recovers readiness after a no-marker audit. Event tests require exported `preview_wait`/`preview_ready` writers in `EVENT_TYPES` and behaviorally prove neither can participate in exclusive-stage pairing; exporting `STAGE_PAIRS` is not required. Route-manifest fixtures require every ordinary/recovery preview input, explicitly reject `source_pr`, `preview_run_id`, `preview_artifact_digest` and every other production/policy-only field, prove ordinary/merged dry-run remains nonterminal, forbid historical dry-run, prove ordinary/merged apply or apply-only historical recovery alone records `dispatched`, refuse all terminal ready IDs, and supersede merged readiness when `main` moves between runs. Runtime-ledger fixtures inject the live `PREVIEW_PROJECT_REF` variable and fresh read immediately before each required run, forbid any `PROJECT_REFS.preview` fallback, and exit 2 for missing credentials, invalid/unset/production-equal identity, variable/transport failure or malformed rows. The Phase A dependency-free `scripts/test_production_migration_guard.py` assertions protect the existing workflow concurrency, target ordering, permissions, dispatch inputs and promotion-procedure semantics without a branch-wide diff assertion or PyYAML.
