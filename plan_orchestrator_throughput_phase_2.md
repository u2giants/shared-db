...
`dispatch-preview.mjs` is the dependency-aware preview front end. It accepts preview only, validates the route and exact current PR head/bundle, then calls `dispatch-shared-migrations.mjs`. The shared operator-side dispatcher is the sanctioned route for both preview and production targets and owns authenticated `gh`, shared admission, exact ref and a closed adapter for every input of `.github/workflows/shared-supabase-migrations.yml`. Input discovery tests compare the adapter with all target inputs—including historical recovery maps—and fail when either side drifts. Ready dispatch records the exact reviewed/current ref plus the complete redacted input-name manifest; the workflow revalidates live state before its exclusive lock.

Before dispatch, the shared operator command proves its worktree is clean and its own bytes plus `dispatch-preview.mjs` match the exact pinned `origin/main` blobs. It creates an immutable admission record containing those blob SHAs, target/ref and input-manifest hash; the workflow validates that record before any target work, and production evidence retains it. Add both scripts to the explicit operator-executable inventory, versioned global invalidators and `PREVIEW_PRODUCER_PATHS` where applicable. Discovery tests enumerate sanctioned dispatch executables and fail on unlisted files; risk-gate tests reject dirty/unpinned local bytes, stale refs and manifest mismatch.
...
Before preview or production dispatch, the shared command acquires `refs/db-coordination/shared-dispatch-admission` with the existing create-if-absent/fenced pattern and proves no queued or in-progress **`workflow_dispatch`** run of `shared-supabase-migrations.yml` exists; pull-request runs are irrelevant. It creates the immutable admission record, dispatches exactly once, reads back the created run/ref/inputs and releases. Add an admission input and workflow-level concurrency expression so an ordinary UI dispatch with the default empty admission ID enters a unique unauthorized group, cannot replace a sanctioned pending run, and fails validation; sanctioned runs alone enter the shared group. Preview and production operator routes both call this helper. Unreadable run state or ambiguous dispatch/readback is `UNVERIFIABLE`.
...
**Verification gate:** transcript-derived fixtures reproduce #1720 waiting behind #1713, automatic guarded redispatch after #1713 rebind, #1720 historical-route selection, and #1646 dependency closure. Dispatcher tests prove wait creates no run; preview front end rejects production while the shared helper owns both sanctioned target routes; every input round-trips; exact clean pinned local bytes/ref/admission readback are mandatory; two preview/production contenders admit one; only queued/in-progress workflow-dispatch runs block; PR validation runs do not; empty/invalid UI admission cannot enter/replace the sanctioned group; response loss and stranded admission recovery are idempotent. A ready preview reaches live validation and exactly one preview lock.
...
- a waker first revalidates the live PR head, canonical bundle and eligibility. On drift, under the owned mutex it creates/readbacks the next generation for current content **before** terminally marking the old generation `superseded`. Recovery treats two unresolved generations under one primary sequence as an interrupted rollover: after live revalidation it retains the newest current generation and terminalizes the stale one; if successor creation never happened, it creates it. Thus a crash between writes cannot remove the issue from the queue. For a current wait, the waker atomically creates `refs/db-reviewer-wait-claims/<wait-id>` before allocation. Release and Step 8 reconciliation contend on that same claim, so one wins. Success/abandon/supersession is recorded once under `refs/db-reviewer-wait-outcomes/<wait-id>/<outcome>`; completed generations are excluded. A dead claimant is recoverable only through existing liveness/fenced proof;
...
- `scripts/orchestrator-flow/dispatch-preview.test.mjs` and `dispatch-shared-migrations.test.mjs`: wait creates no run; exact clean pinned ref; complete target-input passthrough; workflow-dispatch-only run-state filter; shared preview/production admission; unauthorized UI isolation; ambiguous response/stranded-lock reconciliation; preview-front-end refusal of production; ready preview reaches live validation and exactly one preview lock.
