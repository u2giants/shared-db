...
Do not create a new ready ref on every observed `main` commit. Step 8 creates the initial ref when an edge becomes satisfied. Thereafter audit reports a stale derived route without mutation; only the explicit `select-preview-route.mjs --prepare-preview-dispatch <issue>` pre-dispatch command may atomically terminalize the prior unresolved ref and materialize the current one. It scans the batched ready/outcome namespace under the existing short mutex and refuses unless it proves zero or one unresolved ready ref for that issue; more than one is `UNVERIFIABLE`. Historical refs remain append-only evidence, so growth is bounded by real dispatch preparation attempts rather than repository commit rate. No new counter, pause state or per-issue index is introduced.
...
6. for a newly satisfied preview edge, re-run `select-preview-route.mjs`, create/readback the initial immutable ready ref/event, and optionally wake the live sole-orchestrator task with that exact record; later main movement is report-only here and refreshes only through Step 5's explicit `--prepare-preview-dispatch` command; never dispatch a workflow automatically;
...
**Verification gate:** a fixture with #1658/#1645/#1684/#1720/#1646 all protected but externally blocked reports zero active authors and five protected claims, fills up to the proven active cap, then resumes correctly without losing claims. A newly satisfied preview edge creates one exact ready ref/event and no workflow run; no marker or a marker resolving elsewhere reports readiness without mutation; the next matching marker materializes it once. Later main movement reports stale without creating another ref; explicit preparation owns refresh. Closing/cancelling/merging the issue and recording an exact dispatch each produce the single correct terminal outcome; malformed/ambiguous orphans fail closed. Overlapping reconciliation and reviewer claim/outcome recovery are idempotent under two wakers, terminal reservations, stale heads and dead claimants; preview/merge/production refs never gain heartbeat writers.
...
- `scripts/test_production_migration_guard.py` preserves `.github/workflows/shared-supabase-migrations.yml` concurrency, permissions, required contexts, dispatch inputs, literal production needs and deliberately-first safety steps; it pins the procedure's historical-dry-run no-evidence warning; Step 5 introduces no workflow mutation.
...
| Main churn accumulates ready refs or invalidates a dry-run | initial edge materialization plus explicit pre-dispatch refresh only, at most one unresolved ref, new ID needs new dry-run | disable ready persistence; derive route manually from live state |
