...
Also extend `scripts/test_production_migration_guard.py` in Phase A, before Step 5 depends on it. Add parsed/static assertions for the shared workflow's existing concurrency expression, permissions, dispatch-input schema, required job graph and deliberately-first target checks, plus the manual dry-run-then-apply and exact-evidence obligations in `docs/production-promotion-procedure.md`. These behavioral invariants—not a whole-file or branch-wide diff—prove that Phase 2 cannot weaken either contract while allowing unrelated future maintenance.
...
**Verification gate:** transcript fixtures reproduce #1720 waiting behind #1713, #1720 historical-route selection and #1646 dependency closure. Selector/reconcile tests prove the ref-safe `ready-id` changes with issue, head, bundle or route; wait creates no Actions/check/apply artifact; no marker reports readiness without mutation; a matching marker creates/readbacks one exact ready ref/event; two terminal writers produce one outcome ref; stale head/bundle/route supersedes; issue close/cancel/merge terminalizes an orphan from exact proof; repeated reconciliation emits no duplicate; and a later matching orchestrator recovers readiness after a no-marker audit. Event tests require exported `preview_wait`/`preview_ready` writers in `EVENT_TYPES` and behaviorally prove neither can participate in exclusive-stage pairing; exporting `STAGE_PAIRS` is not required. Route-manifest fixtures require every ordinary/recovery preview input, explicitly reject `source_pr`, `preview_run_id`, `preview_artifact_digest` and every other production/policy-only field, prove dry-run remains nonterminal, prove apply/recovery success alone records `dispatched`, and refuse all terminal ready IDs. Runtime-ledger fixtures inject the live `PREVIEW_PROJECT_REF` variable and fresh read immediately before each dispatch and exit 2 for missing credentials, invalid/default/production-equal identity, variable/transport failure or malformed rows. The Phase A `scripts/test_production_migration_guard.py` assertions protect the existing workflow concurrency, target ordering, permissions, dispatch inputs and promotion-procedure semantics without a branch-wide diff assertion.
