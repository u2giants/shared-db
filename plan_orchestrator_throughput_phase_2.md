...
Also extend `scripts/test_production_migration_guard.py` in Phase A, before Step 5 depends on it. Add dependency-free structural/static assertions for the shared workflow's existing concurrency expression, permissions, dispatch-input schema, required job graph and deliberately-first target checks, plus the existing manual route/evidence obligations in `docs/production-promotion-procedure.md`. Preserve that test's no-PyYAML invariant: extend its existing `_steps()` / `_run_block_commands()` helpers and job-slicing idiom rather than adding an optional parser. These behavioral invariants—not a whole-file or branch-wide diff—prove that Phase 2 cannot weaken either contract while allowing unrelated future maintenance. The historical-recovery operator warning belongs with the Phase C selector semantics in Step 5's AGENTS/anti-collision/skill synchronization, not in the production-only procedure.
...
- `AGENTS.md`, `docs/agents/section-4-anti-collision-rules.md`, the synchronized `shared-db-orchestrator` skill and drift fixtures with the historical-recovery apply-only warning.
...
The existing workflow can accept historical inputs with `mode=dry-run`; it runs source validation but skips every migration apply, recovery-proof write and instance binding, then can still produce a green ledger-only dry-run artifact. Phase 2 deliberately does not modify that shared workflow; therefore that combination is never sanctioned evidence. The selector must refuse to emit it, and the Phase C AGENTS/anti-collision/skill instructions plus drift fixtures must warn that it proves nothing. A future workflow-level hard refusal is a separate hardening change, not permission to weaken this plan's no-workflow-change boundary.

The orchestrator uses the repository's existing manual `shared-supabase-migrations.yml` dispatch procedure. Immediately before **each run that the route actually requires** it must re-run `select-preview-route.mjs` against fresh repository-variable, ledger and live state and require the stored head, bundle, route, route context, allowlist and claim/recovery identity fields to match exactly; for both merged rehearsal and historical rebind this includes proving `commit_sha` is still the current `main` tip. `mode` advances only under the existing route procedure. Any drift, including main-tip movement between dry-run and apply, first supersedes the stale ready item and returns `UNVERIFIABLE`; recomputation uses the new `commit_sha` route context and therefore a new ready ID, never the terminal old ID. A successful ordinary/merged dry-run is reusable only by apply for that exact ready ID. A successor ready ID starts with no dry-run evidence and must perform its own dry-run before apply. Create terminal `dispatched` only after the route's completing apply or apply-only historical-recovery run has exact successful evidence. A ready item with any existing terminal outcome is never dispatched again. Dispatch remains deliberately manual and serialized. “Automatic resume” means automatic dependency detection, durable readiness and live-task wake-up—not automatic workflow dispatch. If no orchestrator is live, `--queue-audit` derives/reports readiness only and mutates nothing; the next sole-orchestrator with a matching live marker recomputes and creates/readbacks the ready ref once.

Do not create a new ready ref on every observed `main` commit. Audit reports a stale derived route without mutation; a matching live orchestrator supersedes/materializes only when an operator is about to act. Keep at most one unresolved ready ref per issue, batch reads, and pause that issue as `UNVERIFIABLE` after three pre-completion route-context supersessions until the churn cause is reviewed. Historical refs remain append-only evidence, so growth is bounded by real dispatch attempts rather than repository commit rate.
...
**Verification gate:** transcript fixtures reproduce #1720 waiting behind #1713, #1720 historical-route selection and #1646 dependency closure. Selector/reconcile tests prove the ref-safe `ready-id` changes with issue, head, bundle, route or route-required `commit_sha`; wait creates no Actions/check/apply artifact; no marker reports readiness without mutation; a matching marker creates/readbacks one exact ready ref/event; two terminal writers produce one outcome ref; stale head/bundle/route/context supersedes; issue close/cancel/merge terminalizes an orphan from exact proof; repeated reconciliation emits no duplicate; and a later matching orchestrator recovers readiness after a no-marker audit. Event tests require exported `preview_wait`/`preview_ready` writers in `EVENT_TYPES` and behaviorally prove neither can participate in exclusive-stage pairing; exporting `STAGE_PAIRS` is not required. Route-manifest fixtures require every ordinary/recovery preview input, explicitly reject `source_pr`, `preview_run_id`, `preview_artifact_digest` and every other production/policy-only field, prove ordinary/merged dry-run remains nonterminal, forbid and warn on historical dry-run, prove ordinary/merged apply or apply-only historical recovery alone records `dispatched`, refuse all terminal ready IDs, and give merged **and historical** readiness a new ID with no inherited dry-run when current `main` moves. Churn fixtures prove audit-only main movement creates no refs, one unresolved ref per issue, attempt-bound supersession and the three-supersession pause. Runtime-ledger fixtures inject the live `PREVIEW_PROJECT_REF` variable and fresh read immediately before each required run, allow literal import only for equality comparison, forbid any `PROJECT_REFS.preview` query fallback, fail on disagreement with the drift literal, and exit 2 for missing capability/credentials, invalid/unset/production-equal identity, variable/transport failure or malformed rows. Compatibility tests retain the drift module's named exports and required-CI behavior; discovery tests require the new helper in the global invalidator inventory. The Phase A dependency-free `scripts/test_production_migration_guard.py` assertions protect the existing workflow concurrency, target ordering, permissions, dispatch inputs and production-procedure semantics without a branch-wide diff assertion or PyYAML; Phase C drift tests pin the historical-dry-run warning in operator instructions.
...
Before the Phase C fresh-session cut, update `docs/agents/section-4-anti-collision-rules.md`, `AGENTS.md`, the `shared-db-orchestrator` skill and drift fixtures for read-only preview dependency decisions, `PREVIEW_READY` wake-up, the mandatory immediate pre-dispatch selector plus fresh-ledger recheck, unchanged manual dispatch, execution-key reservation, overflow and durable review-wait lifecycle. Do not defer these operator-visible semantics to Step 10.
...
The same reconcile pass closes readiness lifecycle gaps: exact issue/PR close, merge, cancellation or dispatch evidence creates the sole terminal outcome; stale head/bundle/route creates `superseded`; malformed or ambiguous orphan state is reported as `UNVERIFIABLE`. Historical ready refs remain append-only evidence and are never deleted.

**Dependencies:** Steps 2, 5 and 7.
**Verification gate:** a fixture with #1658/#1645/#1684/#1720/#1646 all protected but externally blocked reports zero active authors and five protected claims, fills up to the proven active cap, then resumes correctly without losing claims. A satisfied preview edge creates one exact ready ref/event and no workflow run; no marker or a marker resolving elsewhere reports readiness without mutation; the next matching marker materializes it once. Closing/cancelling/merging the issue and recording an exact dispatch each produce the single correct terminal outcome; malformed/ambiguous orphans fail closed. Overlapping reconciliation and reviewer claim/outcome recovery are idempotent under two wakers, terminal reservations, stale heads and dead claimants; preview/merge/production refs never gain heartbeat writers.
...
| Main churn accumulates ready refs or invalidates a dry-run | materialize only on operator action, one unresolved ref, new ID needs new dry-run, pause after three supersessions | disable ready persistence; derive route manually from live state |
