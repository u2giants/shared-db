...
Preserve the existing workflow-level concurrency expression's pull-request per-ref branch and global non-PR branch verbatim, including `cancel-in-progress: false`; do not key concurrency on admission ID. An admission input is validated in the workflow's first dispatch-only guard, before project selection, allowlist parsing, target work or exclusive-lease acquisition. Missing/mismatched/consumed admission fails. Direct UI/API dispatch is unsupported and fails this guard, but no claim is made that operator admission authenticates itself—the enforcing workflow re-derives truth.
...
**Verification gate:** transcript fixtures reproduce #1720/#1713 and #1646 ordering. Dispatcher tests prove wait creates no run; preview front end rejects production while the shared helper owns both sanctioned target routes; every input round-trips; two contenders admit one; only queued/in-progress workflow-dispatch runs block; PR runs do not; response loss and stranded-lock recovery are idempotent. Static workflow tests preserve the existing concurrency expression byte-for-byte and prove absent/mismatched/consumed admission fails before project/allowlist/lease work. A ready preview reaches live validation and exactly one preview lock; production procedure tests invoke the same helper.
