...
begin
  ...
  if p_captured_at is null
     ...
     or (p_expected_row_count = 0 and p_target <> 'wb_franchise_property_evidence')
...
begin
  ...
  if (
       h.target = 'wb_franchise_property_evidence'
       and h.expected_row_count = 0
       and chunks = 0
       and maxchunk = 0
       and total = 0
     ) then
    null;
  elsif chunks=0 or maxchunk<>chunks or total<>h.expected_row_count then
    raise exception 'Warner finalize refused: incomplete chunk set.' using errcode='P0001';
  end if;

  select encode(sha256(convert_to(coalesce(string_agg(chunk_sha256,E'\n' order by chunk_number),''),'UTF8')),'hex')
  ...
  select jsonb_build_object(
    ...
    'rows', coalesce(jsonb_agg(e.value order by c.chunk_number,e.ord),'[]'::jsonb)
  ) into snap
...
begin
  ...
  if p_target='wb_asset_normalized' then
    ...
    begin
      perform nullif(btrim(p_row->>'file_size_bytes'),'')::bigint where p_row ? 'file_size_bytes';
      perform nullif(btrim(p_row->>'source_created_at'),'')::timestamptz where p_row ? 'source_created_at';
      perform nullif(btrim(p_row->>'source_modified_at'),'')::timestamptz where p_row ? 'source_modified_at';
...
begin
 ...
 if jsonb_typeof(p_snapshot)<>'object' or jsonb_typeof(p_snapshot->'rows')<>'array' or (jsonb_array_length(p_snapshot->'rows')=0 and p_target<>'wb_franchise_property_evidence') then raise exception 'Warner normalized import refused: invalid snapshot shape.' using errcode='P0001'; end if;
 ...
 for x in select value from jsonb_array_elements(p_snapshot->'rows') loop
  if p_target='wb_franchise' then
   ...
   values(x->>'source_namespace',x->>'source_id',nullif(btrim(x->>'warner_asset_id'),''),x->>'file_name',x->>'source_path',nullif(btrim(x->>'season'),''),nullif(btrim(x->>'file_size_bytes'),'')::bigint,nullif(btrim(x->>'source_created_at'),'')::timestamptz,nullif(btrim(x->>'source_modified_at'),'')::timestamptz,cid,x->>'source_url',x,encode(sha256(convert_to(x::text,'UTF8')),'hex'))
