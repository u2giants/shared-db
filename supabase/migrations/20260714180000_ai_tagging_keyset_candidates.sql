-- Bound PopDAM AI-tag candidate discovery to an index-backed keyset page.
-- The RPC is intentionally service-role-only: it is worker infrastructure,
-- not a browser-facing DAM search contract.

create index if not exists idx_assets_ai_tag_untagged_candidates
...
create index if not exists idx_assets_ai_tag_all_candidates
  on public.assets (primary_sort_tier, id)
  include (thumbnail_url, filename, relative_path, style_group_id)
  where is_deleted = false
    and thumbnail_url is not null
...
create index if not exists idx_assets_ai_tag_tagged_groups
...
begin
  ...
  if p_mode = 'untagged' then
    ...
    where candidate.is_deleted = false
      ...
      -- Preserve the worker's existing .neq('status', 'tagged') null semantics.
...
revoke all on function public.get_ai_tag_candidates(text, integer, integer, uuid, uuid[]) from public;
revoke all on function public.get_ai_tag_candidates(text, integer, integer, uuid, uuid[]) from anon;
revoke all on function public.get_ai_tag_candidates(text, integer, integer, uuid, uuid[]) from authenticated;
grant execute on function public.get_ai_tag_candidates(text, integer, integer, uuid, uuid[]) to service_role;
