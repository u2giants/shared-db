...
begin
 ...
 if p_captured_at is null or p_snapshot_sha256 !~ '^[0-9a-f]{64}$' or p_expected_row_count<=0 or btrim(coalesce(p_private_source_commit,''))='' or btrim(coalesce(p_captured_by,''))='' or btrim(coalesce(p_source_url,''))='' or p_source_url~'[?#]' then raise exception 'Warner capture refused: invalid manifest metadata.' using errcode='P0001'; end if;
