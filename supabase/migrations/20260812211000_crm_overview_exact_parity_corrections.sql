-- CRM Sidebar + Overview exact-parity correction.
--
-- The first preview rehearsal applied 20260812130000 before independent review
-- found three behavior mismatches. Never rewrite an applied migration, even on
-- preview, so this later CREATE OR REPLACE migration preserves the ledger and
-- corrects the function bodies. It limits email-derived displays to the newest
-- 500 messages, uses rolling seven-day chart windows, and preserves the current
-- pending-approval order.
--
-- Phase 7A of the CRM audit remediation replaces the browser-side counting in
-- popcrm-web's `useCrmStatsQuery` (queries.ts), which fetches full tables for six
-- domains plus the newest 500 emails, and counts/sorts them client-side, with
...
--    to `total` and the math is auditable. The input is deliberately limited to
--    the newest 500 messages, matching fetchEmailMessages(-1), which calls
--    api.crm_email_routing_recent with its hard-coded 500-row cap.
-- ---------------------------------------------------------------------------
...
begin
  ...
  with recent_emails as (
    select src.routing_status
      from crm.email_message src
     order by src.received_at desc nulls last, src.id
     limit 500
  )
  select
    count(*) as total,
...
comment on function api.crm_overview_email_counts() is
  'CRM Overview newest-500 email aggregate: total messages, needs_routing, and per-routing-status counts for the routing-health donut. Hard-gated on app.has_app_access(''crm'').';
...
--    email-volume chart. This mirrors buildWeeklyVolume: adjacent rolling
--    seven-day windows ending at one stable statement_timestamp() boundary,
--    rather than calendar weeks. Only the newest 500 messages participate,
--    matching fetchEmailMessages(-1). p_weeks defaults to 12 and is clamped
--    to [1,24]. `routed` = messages with routing_status = 'ROUTED'.
-- ---------------------------------------------------------------------------
...
declare
  ...
  v_boundary timestamptz := statement_timestamp();
begin
  if not app.has_app_access('crm') then
    raise exception 'crm: not authorized' using errcode = 'insufficient_privilege';
  end if;
  ...
  return query
  with recent_emails as (
    select src.id, src.received_at, src.routing_status
      from crm.email_message src
     order by src.received_at desc nulls last, src.id
     limit 500
  ),
  weeks as (
    select
      v_boundary - (v_weeks - n) * interval '7 days' as week_start,
      v_boundary - (v_weeks - n - 1) * interval '7 days' as week_end
  ...
  select w.week_start::date as week_start,
    ...
    from weeks w
    left join recent_emails e
      on e.received_at >= w.week_start
     and e.received_at <  w.week_end
...
comment on function api.crm_overview_email_volume(integer) is
  'CRM Overview newest-500 email volume series in rolling seven-day windows ending at one stable statement timestamp. Hard-gated on app.has_app_access(''crm''). p_weeks clamped to [1,24].';
...
--    Filtering happens after selecting the newest 500 messages, matching the
--    browser's fetchEmailMessages(-1).filter(needsRouting).slice(0, 6).
...
begin
  ...
  return query
  with recent_emails as (
    select src.id, src.subject, src.sender, src.routing_status, src.received_at
      from crm.email_message src
     order by src.received_at desc nulls last, src.id
     limit 500
  )
...
comment on function api.crm_overview_recent_unrouted(integer) is
  'CRM Overview "Needs routing" panel: unrouted subset of the newest 500 emails (rendered columns only). Hard-gated on app.has_app_access(''crm''). p_limit clamped to [1,100], default 6.';
...
begin
  ...
  select a.id,
   ...
   order by a.stage asc nulls last, a.submitted_date desc nulls last, a.id
   limit v_limit;
end;
$$;
...
comment on function api.crm_overview_pending_approvals(integer) is
  'CRM Overview pending approvals in the browser list order (stage ascending, submitted date descending, then id; rendered columns only). Hard-gated on app.has_app_access(''crm''). p_limit clamped to [1,100], default 6.';
