-- Step 7A — DEFINITIVE corrected plm.promote_coldlion_source_owned.
-- Plan: plan_coldlion_licensor_property_accelerated_cutover.md, Step 7A items 3-4.
--
-- This is the third and final migration in the Step 7A promotion set, and it supersedes the
-- function body shipped by 20260729230000 and 20260729234500. Neither of those files is
-- edited: both were already applied to preview rjyboqwcdzcocqgmsyel, and AGENTS.md §4 rule 4
-- forbids editing an applied migration. That rule is also the only thing that works here —
-- preview's ledger already records both versions, so editing either file could never repair
-- preview because the CLI would never re-run it. Forward correction is the only honest
-- option. Same pattern as 20260727223000, 20260727224500 and 20260728134500.
--
-- Only plm.promote_coldlion_source_owned changes. No table, index, policy, grant, trigger or
-- other function is touched, and this migration reads and writes no data.
--
-- THE THREE FAULTS, ALL FOUND BY THE STEP 7A PREVIEW REHEARSAL BEFORE ANY PRODUCTION
-- APPROVAL WAS REQUESTED. That is exactly what the rehearsal exists for.
--
-- FAULT 1 (fatal, 42703) — fixed in 20260729234500, retained here.
--   The decision CTE referenced its own select-list alias `m.present_this_cycle` inside a
--   WHERE clause. PostgreSQL does not expose select-list aliases to WHERE, so the function
--   raised on EVERY call and had never once executed.
--
-- FAULT 2 (latent, and the dangerous one) — fixed in 20260729234500, retained here.
--   The collision rule quarantined any canonical row reachable from more than one typed key.
--   Rehearsed against preview it quarantined 542 of 542 approved rows — the ENTIRE recurring
--   feed — because multi-key fan-in is the APPROVED DESIGN: Albert's approved Phase 4 mapping
--   deliberately points 542 ColdLion source rows at 271 canonical rows, and the same property
--   legitimately exists under both CW001 and SP001. Behind Fault 1 this would have looked
--   "safe" while silently promoting nothing, forever.
--   The corrected rule quarantines fan-in ONLY when the contributing source rows propose
...
--
-- FAULT 3 (fatal, 42702) — fixed HERE.
--   `sync_run_id` is simultaneously a RETURNS TABLE output column of this function and a
--   real column of plm.coldlion_promotion_audit, so the unqualified references in the two
--   counting sub-selects were ambiguous between a PL/pgSQL variable and a table column. Both
--   are now table-qualified. Fault 3 was only reachable once Faults 1 and 2 were fixed, which
--   is why it appeared on the second rehearsal cycle rather than the first.
--
-- PROVEN: executed against preview inside a rolled-back transaction and then as a real cycle.
-- Result 560 source rows / 542 linked / 542 unchanged / 18 quarantined (new_source_record,
-- the deliberately unapproved ColdLion-only candidates) / 0 protected violations — matching
-- the independent JavaScript planner in tools/coldlion-recurring-promotion.mjs exactly.
