...
-- The unchecked implementation is private. Materialize the public entitlement
-- decision once, then reuse the proven inlinable filter body without evaluating
-- app access while assets are produced.
...
as $$
  ...
  select a.*
  from authorized
  cross join lateral public.filter_effective_assets_unchecked_1703(p_filters) a
  where authorized.ok;
$$;
