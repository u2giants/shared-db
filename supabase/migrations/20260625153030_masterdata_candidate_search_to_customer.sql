-- Repoint the PopDAM Master Data candidate search to core.customer.
--
-- public.search_style_tracker_link_candidates (the RPC the PopDAM Master Data
-- page calls) was authored against the old core.company table in three preview-
-- only migrations (20260625143000/144500/150000) that were never committed and
-- whose workstream was stopped because it built on the messy core.company hub.
--
-- This migration recreates the function against the corrected schema. The ONLY
-- changes vs. the preview definition are in the 'customer' branch:
--   * from core.company  -> from core.customer
--   * target_table label 'company' -> 'customer'
-- The licensor/property/factory/sku branches are unchanged. The customer branch
-- already filters to designflow_plm source refs, so it inherently returns only
-- PLM-backed (active) customers in the new model -- potential customers (no ERP
-- ref) and ingested email noise (now in crm.ingested_domain, not core.customer)
-- are excluded automatically.
--
-- See docs/fix_masterdata.md for the full guidance to the Master Data session.
