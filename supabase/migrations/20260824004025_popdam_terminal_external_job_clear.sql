-- Issue u2giants/shared-db#1211
-- Forward-only replacement of public.update_bulk_operation. The prior applied
-- migration remains untouched. This copy preserves every existing guard and adds
-- only the receipt-proven completed external_job clear described by the issue.
...
declare
  ...
  v_terminal_clear boolean := false;
  v_clear_requested boolean := false;
...
begin
  ...
  -- ==========================================================================
  v_clear_requested := coalesce(
    v_in_job -> 'clear_after_reconciliation' = 'true'::jsonb,
    false);
  if v_in_job is not null and v_in_job ? 'clear_after_reconciliation' then
    v_in_job   := v_in_job - 'clear_after_reconciliation';
  ...
  begin
    ...
    if v_stored_rev is distinct from p_expected_revision then
      if v_clear_requested then
        raise exception
          'update_bulk_operation: terminal clear for "%" refused -- expected revision % does not match current revision %.',
          p_op_key, p_expected_revision, v_stored_rev
          using errcode = '55000';
      end if;
    ...
    -- The only permitted pointer clear is an exact, receipt-proven acknowledgement
    -- of a job that is already durably completed. Both control fields were stripped
    -- above, so exact JSON equality proves that no stored job fact changed.
    if v_clear_requested then
      if v_stored_ambig then
        raise exception
          'update_bulk_operation: terminal clear for "%" refused -- ambiguous submissions cannot be cleared by this contract.',
          p_op_key using errcode = '55000';
      end if;
      if v_stored_phase is distinct from 'completed' then
        raise exception
          'update_bulk_operation: terminal clear for "%" refused -- stored external_job phase is %, not completed.',
          p_op_key, coalesce(v_stored_phase, '<none>') using errcode = '55000';
      end if;
      if v_stored_batch is null or v_in_batch is distinct from v_stored_batch then
        raise exception
          'update_bulk_operation: terminal clear for "%" refused -- provider_batch_id does not exactly match the stored provider job.',
          p_op_key using errcode = '55000';
      end if;
      if not v_token_ok then
        raise exception
          'update_bulk_operation: terminal clear for "%" refused -- the lease receipt is missing or does not match.',
          p_op_key using errcode = '55000';
      end if;
      if p_submission_owner is not null or p_lease_seconds is not null then
        raise exception
          'update_bulk_operation: terminal clear for "%" refused -- clearing and claiming a lease cannot be combined.',
          p_op_key using errcode = '55000';
      end if;
      if v_in_job is distinct from v_stored_job then
        raise exception
          'update_bulk_operation: terminal clear for "%" refused -- the offered external_job is not the exact stored completed job.',
          p_op_key using errcode = '55000';
      end if;

      v_terminal_clear := true;
      p_op_state := p_op_state - 'external_job';
      v_in_job := null;
    end if;
    ...
    if p_submission_owner is null then
      if v_stored_owner is not null
         ...
         and v_in_job is null
         and not v_terminal_clear then
    ...
    -- ------------------------------------------------------------------------
    if v_in_job is null and v_pointer and not v_terminal_clear then
      v_ok := false; v_reason := 'external_job_protected';
      exit guarded;
    end if;
