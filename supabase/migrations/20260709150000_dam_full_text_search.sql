-- DAM: indexed full-text search for asset metadata and extracted PDF text.
--
-- The frontend uses these RPCs to turn a library search term into matching
-- asset/style-group IDs, then applies the existing filters, sorting, and paging.

CREATE INDEX IF NOT EXISTS idx_assets_full_text_search
ON public.assets
USING gin (
  to_tsvector(
    'simple',
    (coalesce(filename, '') || ' ' || coalesce(relative_path, '') || ' ' || coalesce(cover_description, '') || ' ' || coalesce(ai_description, '') || ' ' || coalesce(scene_description, '') || ' ' || coalesce(customer, '') || ' ' || coalesce(program, '') || ' ' || coalesce(licensor_name, '') || ' ' || coalesce(property_name, '') || ' ' || coalesce(product_category, ''))
  )
)
WHERE is_deleted = false;

CREATE INDEX IF NOT EXISTS idx_style_groups_full_text_search
ON public.style_groups
USING gin (
  to_tsvector(
    'simple',
    (coalesce(sku, '') || ' ' || coalesce(folder_path, '') || ' ' || coalesce(cover_description, '') || ' ' || coalesce(customer, '') || ' ' || coalesce(program, '') || ' ' || coalesce(licensor_name, '') || ' ' || coalesce(property_name, '') || ' ' || coalesce(product_category, ''))
  )
);

CREATE INDEX IF NOT EXISTS idx_pdf_text_samples_extracted_text_search
ON public.pdf_text_samples
