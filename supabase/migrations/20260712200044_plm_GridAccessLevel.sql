-- Additive: create DesignFlow table in shared schema plm.
-- Mapped from DesignFlow `GridAccessLevel` per
-- docs/designflow-master-data-migration/designflow-schema-segregation.md section 4.3.
-- Purpose: Column access levels
-- Branch: db-migration. Apply on preview first (shared-db-gatekeeper / AGENTS.md).
--
CREATE TABLE IF NOT EXISTS plm."GridAccessLevel" (
    id integer NOT NULL,
    col_id character varying,
    col_access_level character varying,
    access boolean,
    grid_id character varying
);

DO $$ BEGIN
  ALTER TABLE ONLY plm."GridAccessLevel"
    ADD CONSTRAINT "RFQCustomLayout_pkey" PRIMARY KEY (id);
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;

COMMENT ON TABLE plm."GridAccessLevel" IS 'Column access levels';
