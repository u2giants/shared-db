-- ColdLion recurring promotion: remove the DEAD in-function failure-recording code.
--
-- Plan: plan_coldlion_licensor_property_accelerated_cutover.md, Step 7A.
-- Forward correction of plm.promote_coldlion_source_owned only, per AGENTS.md section 4
-- rule 4 — 20260729230000, 20260729234500, 20260729235500 and 20260730000500 are applied to
-- preview rjyboqwcdzcocqgmsyel and are therefore immutable. No table, index, policy, grant,
-- trigger or other function changes here; this migration reads and writes no data, and the
-- function's behaviour is byte-for-byte identical to 20260730000500 apart from the removal.
--
-- WHY (found reviewing merged PR #331, merge commit 0798c095)
-- ----------------------------------------------------------
-- Every version of this function since 20260729230000 carried code that LOOKED like durable
-- failure recording and could never run:
--
--   * a body-level `exception when others` handler whose only job was
--         update ingest.sync_run set status = 'failed' ... where id = v_run_id;
--   * two inline copies of the same UPDATE immediately before a RAISE (sections 5.4 and 5.6).
--
-- In PL/pgSQL a block with an exception handler is an implicit SUBTRANSACTION. When an error
-- reaches the handler, everything the block did is already rolled back — including the
-- section 5.3 INSERT that created the ingest.sync_run row. The UPDATE therefore runs against a
-- row that no longer exists and matches zero rows, every time, without complaint. The inline
-- copies are dead for the same reason: the RAISE that follows them discards them, whether it is
-- caught by the body handler or propagates straight to the caller.
--
-- This was never a functional bug — failures ARE recorded, just not here (below). It is a
-- HAZARD, and this repository treats the hazard class as a first-class defect: the dead code
-- states, in the most authoritative place a future session will look, that the database records
-- its own failures. A session that believes that may then delete the code that actually does
-- the job as "redundant", and reintroduce a genuine silent failure.
--
-- WHERE DURABLE FAILURE RECORDING ACTUALLY LIVES — DO NOT DELETE IT
-- ----------------------------------------------------------------
-- `tools/promote-coldlion-source-owned.mjs` owns it, OUT OF BAND, and it must stay that way.
-- Its catch block runs AFTER the aborted promotion transaction is gone, and issues two SEPARATE
...
--
-- A separate transaction is not a workaround; it is the ONLY place this can work. Nothing
-- inside the aborted transaction can leave a durable trace of its own abort. If a future change
-- wants failure recording back in the database, it belongs in an `autonomous`-style separate
-- connection (e.g. dblink) — never in an in-function exception handler.
