-- Issue #679 -- Forward-repair the Paramount loader above the promotable structural set.
--
-- WHY THIS FILE EXISTS, AND WHY IT IS NOT 20260825094455. 20260825094455 carries this
-- exact repaired body and is merged, but it can never reach production: its only preview
-- apply (run 32834195883) ran on PR #1465's branch commit c118a3c8, which a squash merge
-- left outside main's history, so the production business-risk gate refuses its byte
-- binding (measured, run 32850264285). The same defect retired 20260814223552, replaced
-- by 20260825124200. This file re-derives the body under a version whose rehearsal can be
-- proved, so 20260825094455 is superseded and must never be promoted.
--
-- WHY A LOADER MIGRATION IS STILL REQUIRED. 20260814193351 and 20260814213043 each rewrite
-- plm.load_pmt_capture_chunk on their way past. Production today already runs the repaired
-- body (20260824135515), so applying those two WITHOUT a forward repair above them would
-- leave the loader at the second 2026-08-14 rewrite and silently restore the issue #1418
-- JSON-null defect. Catalog verification cannot catch that: the function exists either way.
--
-- REQUIRED PRODUCTION ORDER. The bounded window is exactly:
--   20260814193351 -> 20260814213043 -> 20260825124200 -> 20260825130500.
-- 20260814223552 and 20260825094455 are replaced by this set and must never be promoted.
--
-- WHAT WAS BROKEN. plm.load_pmt_capture_chunk selected `r->'raw_value'` for the
-- pmt_asset_metadata_value target. When the private builder correctly represents
-- an ABSENT optional nested remainder as the JSON literal `null`, the key is
-- PRESENT and `->` returns JSONB `null` -- not SQL NULL. pmt_amv_raw_value_shape_chk
-- allows SQL NULL or a safe JSON object, so JSONB `null` is refused and the whole
-- capture fails closed on its first value chunk (preview capture
-- 58cfc82c-571e-4bc9-b9d5-327ea9379782, 2026-08-24).
--
-- THE REPAIR. Normalize JSONB `null` to SQL NULL at the loader boundary with
-- `nullif(r->'raw_value', 'null'::jsonb)`. Nothing else changes:
--
--   * pmt_amv_raw_value_shape_chk is NOT dropped, weakened, disabled or bypassed.
--     A non-object raw_value (a string, a number, an array) and any URL- or
--     credential-shaped key are still refused exactly as before.
--   * `->` (keeps jsonb) is still used, never `->>` -- see the in-body note.
--   * The pmt_asset_metadata_value target is NOT omitted; it is still loaded, with
--     every other column written exactly as before.
--
-- This is a full re-derivation of the CURRENT function body on main (20260825094455,
-- itself derived from 20260824135515), not a copy or merge of any older rewrite.
...
begin
  ...
  if p_target = 'pmt_capture_batch' then
    ...
    -- ISSUE #1418: `nullif(..., 'null'::jsonb)`. A PRESENT key whose value is the
    -- JSON literal null yields JSONB `null` from `->`, which is NOT SQL NULL and is
    -- refused by pmt_amv_raw_value_shape_chk. An absent optional remainder must land
    -- as SQL NULL. This normalizes ONLY the JSON-null case: an object still stores as
    -- an object, and a string/number/array/blocked-key value is still rejected.
    ...
    select p_capture_id,
      ...
      nullif(r->'raw_value', 'null'::jsonb), md5(r::text)
    from jsonb_array_elements(p_rows) r;
...
'Issue #1418: a metadata raw_value that arrives as the JSON literal null is normalized to SQL '
'NULL at this boundary, because JSONB null is not SQL NULL and pmt_amv_raw_value_shape_chk '
'accepts only SQL NULL or a safe JSON object. The constraint itself is unchanged. Issue #1459 '
're-derived this exact repaired body above the three 20260814 rewrites so applying that trio '
'cannot silently restore the pre-repair loader.';
...
-- Definition and privilege verification only. Behavioral contracts use synthetic rows and roll
-- back independently; applying this migration never touches captured data.
do $$
declare
...
begin
  ...
  if v_def not like '%pmt_metadata_element%'
     or v_def not like '%pmt_asset_metadata_value%'
    ...
    raise exception 'Paramount forward repair lost one of the three loader rewrites';
  end if;
  if has_function_privilege('anon',
    ...
    raise exception 'Paramount loader grants changed';
  end if;
  raise notice 'Paramount loader forward repair verified above all three rewrites';
end;
$$;
