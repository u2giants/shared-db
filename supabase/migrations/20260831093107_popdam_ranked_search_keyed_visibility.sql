-- #1703 forward 2: remove candidate x library OR joins from ranked search.
-- derived-from: 20260831074401

create index if not exists assets_style_group_id_active_idx
  on public.assets (style_group_id, id)
  where is_deleted = false and style_group_id is not null;

-- The legacy count entry point and the effective entry point describe the same
-- effective library. Keep one implementation so the legacy wrapper cannot take
-- the older production path that exceeded the edge timeout.
...
as $$
  with params as materialized (
    ...
    select distinct a.* from visible_assets a
...
comment on function public.search_dam_documents(text,jsonb,int,int,text[],extensions.vector(384),real) is
  'DAM-entitled ranked search: candidate asset keys drive effective visibility before totals, facets, and pagination.';
