-- Add exact Control Room headline metrics to the shared department aggregate.

create index if not exists pim_product_business_unit_idx
  on pim.product (lower(coalesce(metadata ->> 'business_unit', metadata ->> 'department', '')));

create index if not exists pim_project_business_unit_idx
  on pim.project (lower(coalesce(metadata ->> 'business_unit', metadata ->> 'department', '')));
...
begin
  ...
  products as materialized (
    select p.id, p.project_id, p.design_id, p.stage, p.lifecycle_status,
      m.blocker_reason, m.waiting_on, m.next_action, m.next_owner_name,
      m.next_owner_role_name, m.evidence_gaps, m.pps_requested_date,
      m.on_shelf_date, m.closure_reason, m.risk_level, m.priority
    from pim.product p
    cross join lateral jsonb_to_record(p.metadata) as m(
      blocker_reason text, waiting_on text, next_action text,
      next_owner_name text, next_owner_role_name text, evidence_gaps jsonb,
      pps_requested_date text, on_shelf_date text, closure_reason text,
      risk_level text, priority text
    )
  ...
  closure_buckets as (
    select coalesce(nullif(p.closure_reason, ''), 'Unknown') key, count(*)::int count
    from products p
    where nullif(p.closure_reason, '') is not null
    group by 1 order by count(*) desc, 1
  ),
  risk_buckets as (
    select coalesce(nullif(p.risk_level, ''), 'Unknown') key, count(*)::int count
    from products p
    where nullif(p.risk_level, '') is not null
    group by 1 order by count(*) desc, 1
  ),
  waiting_buckets as (
    select coalesce(nullif(p.waiting_on, ''), 'Unknown') key, count(*)::int count
    from products p
    where nullif(p.waiting_on, '') is not null
    group by 1 order by count(*) desc, 1
  )
  select jsonb_build_object(
    ...
    'operational', jsonb_build_object(
      'blocked', (select count(*) from products p where nullif(p.blocker_reason, '') is not null or nullif(p.waiting_on, '') is not null or lower(coalesce(p.lifecycle_status, '')) in ('blocked','waiting')),
      'urgent', (select count(*) from products p where lower(coalesce(p.priority,'')) in ('urgent','high')),
      'active_projects', (select count(*) from projects pr where lower(coalesce(pr.status,''))='active'),
      'ownership_gaps', (select count(*) from products p where nullif(p.next_action, '') is not null and nullif(p.next_owner_name, '') is null and nullif(p.next_owner_role_name, '') is null and nullif(p.waiting_on, '') is null),
      'evidence_gaps', (select count(*) from products p where jsonb_array_length(case when jsonb_typeof(p.evidence_gaps) = 'array' then p.evidence_gaps else '[]'::jsonb end) > 0),
      'overdue_dates', (select count(*) from products p where nullif(p.pps_requested_date,'')::date < current_date or nullif(p.on_shelf_date,'')::date < current_date),
