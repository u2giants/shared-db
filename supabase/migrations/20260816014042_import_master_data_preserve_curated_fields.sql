-- Preserve curated Licensor and Property decisions during DesignFlow PLM imports.
-- Matched canonical rows and curated source-link fields are never replaced. Feed values
-- still land in PLM mirror and raw evidence tables. Global unique matching prevents a
-- curated row under another parent from becoming a duplicate; ambiguity fails loudly.
-- Function body only. No existing rows are rewritten by this migration.
...
begin
  ...
  loop
    ...
    if core_licensor_id is null and v_source_code is not null then
      select count(*), (array_agg(l.id order by l.created_at))[1]
      into v_match_count, core_licensor_id from core.licensor l where l.code = v_source_code;
      if v_match_count > 1 then raise exception 'PLM licensor code matches multiple curated licensors; refusing source id %', v_source_id; end if;
    end if;

    if core_licensor_id is null then
      select count(*), (array_agg(l.id order by l.created_at))[1]
      into v_match_count, core_licensor_id from core.licensor l where lower(l.name) = lower(v_source_name);
      if v_match_count > 1 then raise exception 'PLM licensor name matches multiple curated licensors; refusing source id %', v_source_id; end if;
    end if;
    ...
    if core_licensor_id is null then
      ...
      -- Matched canonical rows are curated. Incoming values remain evidence only.
      null;
    end if;
    ...
    loop
      ...
      if core_property_id is null and v_source_code is not null then
        ...
        into v_match_count, core_property_id from core.property p where p.code = v_source_code;
        if v_match_count > 1 then raise exception 'PLM property code matches multiple curated properties; refusing source id %', v_source_id; end if;
      end if;

      if core_property_id is null then
        select count(*), (array_agg(p.id order by p.created_at))[1]
        into v_match_count, core_property_id from core.property p where lower(p.name) = lower(v_source_name);
        if v_match_count > 1 then raise exception 'PLM property name matches multiple curated properties; refusing source id %', v_source_id; end if;
      end if;
      ...
      if core_property_id is null then
        ...
        -- Preserve curated parentage, name, code, status, metadata, and all other canonical fields.
        null;
      end if;
...
comment on function plm.import_master_data(jsonb, jsonb) is
  'DesignFlow PLM importer. Matched Licensor and Property canonical fields and curated source-link fields are preserved; incoming values continue into mirror and raw evidence tables.';
