-- Preview correction: keep the materialized product set narrow. Copying the
-- restored ClickUp metadata blob exceeded the eight-second statement budget.
...
begin
  ...
  products as materialized (
    select p.id, p.project_id, p.design_id, p.stage, p.lifecycle_status,
      ...
      p.metadata -> 'evidence_gaps' as evidence_gaps,
