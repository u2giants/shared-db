-- ClickUp incremental task import — RE-ISSUE of the silently skipped 20260728160000.
-- Migration: 20260728174500_clickup_incremental_task_import_reissue.sql
--
-- ##############################################################################
-- # WHY THIS FILE EXISTS — DO NOT "CLEAN UP" THE APPARENT REDUNDANCY.          #
-- ##############################################################################
--
-- This migration re-issues, verbatim and idempotently, the DDL of
-- `20260728160000_clickup_incremental_task_import.sql`, which is present in this
-- repository but was NEVER APPLIED to any database.
--
-- Root cause: TWO migrations were authored with the SAME version `20260728160000`
--   * 20260728160000_popdam_user_tables_foreign_keys.sql   (landed first, commit 0b8425b)
--   * 20260728160000_clickup_incremental_task_import.sql   (landed later, PR #305)
-- Supabase's ledger `supabase_migrations.schema_migrations` keys on the VERSION
-- (the 14-digit timestamp) ALONE — not on the filename. The PopDAM file applied
-- first and claimed version `20260728160000`; the ClickUp file was therefore
-- treated as already-applied and SILENTLY SKIPPED. No error, no warning.
-- (AGENTS.md §4 rule 5 documents this failure mode; the same thing happened on
-- 2026-07-22 with version 20260722220000.)
--
-- Verified in production `qsllyeztdwjgirsysgai` on 2026-07-29 (read-only):
--   * the ledger holds `20260728160000` exactly once, as `popdam_user_tables_foreign_keys`
--   * the PopDAM half IS applied (3 of 3 foreign keys present)
--   * the ClickUp half is ENTIRELY ABSENT: `pim.sync_clickup_tasks` does not exist
--     and none of the new `clickup_*` columns exist on `pim.product`
--   * the skip is CLEAN — zero partial ClickUp artifacts
--
-- How the ledger behaves after this migration:
--   * `20260728160000` stays recorded once, pointing at the PopDAM migration.
--   * `20260728174500` is recorded for THIS file, which is what actually creates
--     the ClickUp objects.
--   * `20260728160000_clickup_incremental_task_import.sql` remains on disk and will
--     NEVER run anywhere. It is deliberately left untouched: AGENTS.md forbids
--     editing or renaming a landed migration, and renaming it would change a version
--     the ledger already holds. `supabase migration repair` was deliberately NOT
--     used — ledger surgery on a shared production database is not auditable;
--     a forward migration is.
--
-- Timestamp choice: `20260728174500` sorts AFTER the last already-landed migration
-- (`20260728171500_db_data_admin_tree_plm_division_names.sql`) and BEFORE
-- `20260728181500_clickup_incremental_task_import_fixes.sql` (PR #311), which
-- depends on the objects created here. Do not renumber either neighbour.
--
...
-- so re-running this file, or running it on a database where the original somehow
-- did apply, is a clean no-op. The ONE intentional deviation from the original file:
-- the one-time backfill in section 2 now records its `ingest.sync_run` audit row only
-- when it actually claimed or skipped something, so a second application does not
-- append a second, meaningless accounting row. In production the backfill claims
-- nothing (there are zero `pim.product` rows with a NULL `external_source`), so this
-- section is a no-op there — PR #311 removes the dead backfill entirely.
--
-- Everything below this line is the original migration's own documentation and DDL.
-- ------------------------------------------------------------------------------
--
-- What this does
-- --------------
...
begin
  ...
  -- Idempotency guard (deviation from the original file, see header): only record the
  -- audit row when the backfill actually did something. Re-running this migration on a
  -- database where nothing is claimable must add no rows at all.
  if v_updated > 0 or coalesce(array_length(v_ambiguous, 1), 0) > 0 then
    ...
    values
      ('clickup', 'clickup_external_id_backfill', 'succeeded', now(), now(),
       ...
       jsonb_build_object(
         'migration', '20260728174500_clickup_incremental_task_import_reissue',
         'reissue_of', '20260728160000_clickup_incremental_task_import',
         'stage', 'backfill',
