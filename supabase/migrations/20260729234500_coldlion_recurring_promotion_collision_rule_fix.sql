-- Step 7A forward CORRECTION of 20260729230000_coldlion_licensor_property_recurring_promotion.sql.
...
-- ----------------------------------------------------------
-- 20260729230000 was applied to PREVIEW rjyboqwcdzcocqgmsyel before its promotion function
-- was exercised. AGENTS.md §4 rule 4 forbids editing a migration that has been applied
-- anywhere, and here that rule is also the only thing that WORKS: preview's
-- supabase_migrations.schema_migrations ledger already records version 20260729230000, so
-- editing that file could never repair preview — the CLI would simply never re-run it. A
-- forward correction is therefore the only honest option, and it keeps the file and the
-- ledger in agreement. This is the same pattern already used by 20260727223000,
-- 20260727224500 and 20260728134500. It is deliberate, not sloppiness.
--
-- WHAT WAS WRONG — both faults were in plm.promote_coldlion_source_owned only. No table,
-- index, policy, grant, trigger or other function from 20260729230000 is changed here, and
-- no data is read or written by this migration.
--
-- FAULT 1 (fatal, 42703). The decision CTE referenced its own select-list alias
-- `m.present_this_cycle` inside a WHERE clause. PostgreSQL does not expose select-list
-- aliases to WHERE, so the function raised on EVERY call and had never once executed. The
-- expression is now repeated instead of aliased.
--
-- FAULT 2 (latent, and far more dangerous). The collision rule quarantined any canonical row
-- reachable from more than one typed key. Rehearsed against preview on 2026-07-29 that rule
-- quarantined 542 of 542 approved rows — the ENTIRE recurring feed — because multi-key
-- fan-in is the APPROVED DESIGN, not a fault: Albert's approved Phase 4 mapping deliberately
-- points 542 ColdLion source rows at 271 canonical rows, and the same property legitimately
-- exists under both CW001 and SP001. Had this shipped behind Fault 1, the feed would have
-- looked "safe" while promoting nothing at all, forever.
-- The corrected rule quarantines fan-in ONLY when the contributing source rows propose
-- DIFFERENT names, because that is the only case where promotion would have to pick a
-- winner. A future session must NOT simplify this back to `key_count > 1`.
--
-- Both faults were found by the Step 7A two-cycle preview rehearsal, before any production
-- approval was requested. That is precisely what the rehearsal exists for.
