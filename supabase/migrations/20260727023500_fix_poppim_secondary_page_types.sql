-- Preview correction: app.profile.email is citext; cast outward-facing RPC
-- columns to their declared text type.
...
begin
  ...
  from pim.project pr
  left join core.customer c on c.id=pr.company_id
...
begin
  ...
  select pr.id,coalesce(pr.display_name,pr.email::text,'Unnamed person'),pr.email::text,pr.avatar_url,pr.status::text,
...
begin
  ...
  with department_targets as materialized (
    ...
    select c.id,'comment'::text kind,c.body,c.target_schema||' / '||c.target_table||' / '||c.target_id,
      c.created_at,coalesce(pr.display_name,pr.email::text) author
    ...
    select a.id,'activity',coalesce(a.summary,a.action),a.target_schema||' / '||a.target_table||' / '||a.target_id,
      a.created_at,coalesce(pr.display_name,pr.email::text)
