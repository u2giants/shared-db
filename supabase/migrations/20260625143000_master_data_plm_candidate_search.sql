-- Route Master Data candidate matching through PLM-backed canonical identities.
-- The existing PopDAM Master Data page calls this public RPC directly. Keep the
-- return contract stable, but stop offering non-PLM customer/licensor noise.
...
begin
  ...
  if p_field_key = 'customer' then
    ...
    select c.target_schema, c.target_table, c.target_id, c.target_label, c.score
    from candidates c
    where use_all
       or c.score >= 0.2
    order by
  ...
  if p_field_key = 'sku' then
    ...
    where use_all
       or c.score >= 0.2
    order by c.score desc, c.target_label
    limit max_rows;
    return;
  end if;
end;
