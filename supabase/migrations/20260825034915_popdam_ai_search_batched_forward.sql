-- Separately timed, atomic forward recovery for the complete PopDAM scoped AI
-- metadata and deterministic tag/character search contract.
-- GitHub: u2giants/shared-db#1474, #1427; u2giants/popdam3#96, #97.
set statement_timeout = '10min';

-- Preview already has the complete contract. Production does not. Keep this
-- transaction-local flag so every direct statement is idempotent on preview.
select set_config(
  'popdam.forward_1474_needed',
...
do $$
begin
  if current_setting('popdam.forward_1474_needed') = 'on' then
...
where current_setting('popdam.forward_1474_needed') = 'on'
  and nullif(btrim(t.tag), '') is not null
...
with ranked as (
  ...
  delete from public.asset_tags t using ranked r
  where t.id=r.id and r.rn>1 returning t.id
)
...
begin
  ...
  with batch as (
    ...
    order by id limit p_limit for update
  ), changed as (
...
-- 512 distinct top-level statements: each has its own unchanged 10-minute
-- timeout, while the surrounding migration transaction remains atomic.
select pg_temp.popdam_forward_1474_normalize_batch(5000);
select pg_temp.popdam_forward_1474_normalize_batch(5000);
select pg_temp.popdam_forward_1474_normalize_batch(5000);
select pg_temp.popdam_forward_1474_normalize_batch(5000);
select pg_temp.popdam_forward_1474_normalize_batch(5000);
...
begin
  if current_setting('popdam.forward_1474_needed') = 'on' and exists (
    ...
    raise exception 'PopDAM normalization exceeded the guarded 2,560,000-row envelope';
  end if;
end $$;

create or replace function pg_temp.popdam_forward_1474_rebuild_batch(p_limit integer)
returns integer language plpgsql as $$
declare v_last uuid; v_count integer; v_next uuid;
begin
  if current_setting('popdam.forward_1474_needed') <> 'on' then return 0; end if;
  ...
  with batch_ids as (
    ...
    order by t.asset_id limit p_limit
  ), desired as (
    ...
    where a.id=d.asset_id and a.tags is distinct from d.tags returning a.id
  )
...
-- 512 separately timed 10,000-asset compatibility batches.
select pg_temp.popdam_forward_1474_rebuild_batch(10000);
select pg_temp.popdam_forward_1474_rebuild_batch(10000);
select pg_temp.popdam_forward_1474_rebuild_batch(10000);
select pg_temp.popdam_forward_1474_rebuild_batch(10000);
select pg_temp.popdam_forward_1474_rebuild_batch(10000);
...
begin
  ...
  if current_setting('popdam.forward_1474_needed') = 'on' and exists (
    ...
    raise exception 'PopDAM compatibility rebuild exceeded the guarded 5,120,000-asset envelope';
  end if;
end $$;
