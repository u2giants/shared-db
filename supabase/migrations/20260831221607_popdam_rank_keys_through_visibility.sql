-- Forward repair for #1703: carry each candidate's rank keys through the
-- existing keyed asset lookup. This removes the quadratic materialized-CTE
-- rescan while preserving exact visibility, totals, facets, and pagination.
-- derived-from: 20260831212757
...
as $$
  with params as materialized (
    ...
    select c.document_type, c.entity_id, c.asset_id, c.style_group_id,
      c.keyword_rank, c.semantic_rank, c.rank,
    ...
    select distinct a.document_type, a.entity_id, a.asset_id,
      a.style_group_id, a.keyword_rank, a.semantic_rank, a.rank
    from visible_assets a
  ), matched_assets as materialized (
    select distinct a.id, a.asset_style_group_id style_group_id,
      a.file_type, a.status, a.workflow_status, a.stage, a.is_licensed
    from visible_assets a
...
comment on function public.search_dam_documents(text,jsonb,int,int,text[],extensions.vector(384),real) is
  'DAM-entitled ranked search: candidate ranks flow through one private keyed visibility pass for exact totals, facets, and pagination.';
