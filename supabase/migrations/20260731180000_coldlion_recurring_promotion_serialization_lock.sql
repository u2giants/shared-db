-- Step 7A - SERIALIZATION GUARD: the recurring promotion had NO mutual exclusion.
-- Plan: plan_coldlion_licensor_property_accelerated_cutover.md, Step 7A item 3.
-- Raised by an independent review of PR #331 (merge commit 0798c095) as a Medium finding.
--
...
-- writes no data. The public.promote_coldlion_source_owned wrapper is unchanged and needs
-- no change: it is a one-statement `language sql` wrapper, so it runs inside the SAME
-- transaction and the transaction-scoped lock taken below covers it.
--
-- REBASED ONTO 20260731163000 (2026-07-31). This migration was originally authored as
-- version 20260731170000 against the body of 20260730000500. Three other sessions authored
-- `create or replace` migrations for this same function at the same time, and
-- `create or replace` is last-writer-wins: keeping the old body would have silently ERASED
-- the dead-failure-recording removal that landed in 20260731163000. The function body below
-- is therefore taken verbatim from 20260731163000 — the current definition on main — with
-- ONLY the serialization guard added on top. Specifically: the two new declarations
-- (v_lock_key, v_lock_acquired), the new section 5.0 block, and the refreshed
-- `comment on function`. Nothing else in the function differs from 20260731163000. In
-- particular the body-level `exception when others` handler stays REMOVED and the three
-- dead `update ingest.sync_run set status = 'failed'` statements stay REMOVED; see the
-- header of 20260731163000 for why they could never run.
--
-- WHERE DURABLE FAILURE RECORDING ACTUALLY LIVES — DO NOT DELETE IT
-- ----------------------------------------------------------------
-- Carried forward from 20260731163000, because this file is now the newest definition of the
-- function and is therefore the first place a future session will look. Failure recording is
-- OUT OF BAND in `tools/promote-coldlion-source-owned.mjs`, and it must stay that way. Its
-- catch block runs AFTER the aborted promotion transaction is gone, and issues two SEPARATE
-- psql transactions that therefore survive the rollback:
--
...
-- "restore" an in-function exception handler here. NOTE the contrast with section 5.0 below:
-- the skipped_already_running row DOES commit, because that path RETURNS normally instead of
-- raising, so nothing rolls it back.
--
-- THE FAULT
-- ---------
-- The recurring promotion is invoked from two places: the scheduled GitHub Actions lane
-- (.github/workflows/coldlion-licensor-property-production.yml, still DISABLED) and manual
-- drills (`node tools/promote-coldlion-source-owned.mjs --apply ... --drill`). Nothing
-- prevented the two from running at the same time, and nothing prevented one scheduled run
-- from overlapping the next if it overran its interval.
--
-- Two concurrent cycles read the same mirror rows and the same approved links, compute two
-- plans from the same snapshot, and then both apply. The damage is not a lost update to
-- core.* - the protected-invariant guard still holds, and the source-owned writes are
-- idempotent - it is BOOKKEEPING. Both cycles open an ingest.sync_run row under
-- coldlion_licensors_properties_promote_source_owned; both write plm.coldlion_promotion_audit
-- rows for the same field on the same entity; the second cycle then reports
-- provenance_refreshes/curated_name_changes for work the first one already did. Anyone
-- reading the audit trail afterwards sees a name promoted twice and cannot tell which run is
-- authoritative. On the production lane that trail is the evidence Albert signs off against.
--
-- THE FIX: pg_try_advisory_xact_lock on a documented constant key, taken as the first
-- statement in the function. Full rationale is inline at 5.0 below. Losing the race is a
-- RECORDED, distinguishable outcome (mode = skipped_already_running, a cancelled
-- ingest.sync_run row), never a silent no-op - and it is deliberately NOT a failure, so it
-- cannot trip the two-consecutive-failure pg_notify breaker in
-- tools/coldlion-sync-common.mjs.
--
-- This migration does NOT enable the production lane and does NOT introduce
-- COLDLION_LICENSOR_PROPERTY_PRODUCTION_ENABLED. Preview-prove only; Step 8 (Albert's
-- production approval) is unchanged and still pending.
