-- Synthetic contract suite for Marvel DCP Vault; contains no licensed source values.
-- =====================================================================================
-- Contract tests for migrations 20260810190000 and 20260810190100 -- the Disney Marvel DCP Vault
...
--       then, with that URL in MARVEL_DCP_TEST_URL:
--         psql "$MARVEL_DCP_TEST_URL" -v ON_ERROR_STOP=1 -f supabase/tests/marvel_dcp_vault_landing_contracts.sql
...
--        The table list is ENUMERATED FROM pg_class, so a plm.marvel_dcp_* table added later
...
--     D  THE NULL-ROLE CASE: plm.marvel_dcp_loader_privilege_ok returns FALSE for NULL/NULL and
...
--        deliberate marvel_dcp_load_exception carve-out (resolution columns stay writable, source
--        fields do not). E8 reads back the STORED waived_at written by
--        plm.close_marvel_dcp_crawl_gap and asserts it is exactly 12:00:00 UTC.
...
declare
  v_tables text[] := array[
    'marvel_dcp_crawl','marvel_dcp_portal_tile','marvel_dcp_style_guide','marvel_dcp_asset','marvel_dcp_crawl_section',
    'marvel_dcp_crawl_gap','marvel_dcp_asset_crawl','marvel_dcp_asset_tile_observation','marvel_dcp_load_exception',
    'marvel_dcp_chunk_ledger'
  ];
  t text;
...
begin
  ...
  where p.pronamespace = 'plm'::regnamespace
    and p.proname in (
      'marvel_dcp_loader_privilege_ok','marvel_dcp_asset_row_hash','marvel_dcp_reject_completed_crawl_change',
      'marvel_dcp_load_exception_freeze',
      'marvel_dcp_reject_completed_source_field_change','marvel_dcp_crawl_freeze','begin_marvel_dcp_crawl',
      'open_marvel_dcp_crawl_section','close_marvel_dcp_crawl_section','load_marvel_dcp_asset_chunk',
      'record_marvel_dcp_crawl_gap','close_marvel_dcp_crawl_gap','finalize_marvel_dcp_crawl','fail_marvel_dcp_crawl'
    );
  if v_fns <> 14 then
    raise exception 'A FAILED: expected 14 plm.marvel_dcp_* / DCP loader functions in pg_proc, '
...
--
-- The table list is READ FROM pg_class, not hard-coded, so a plm.marvel_dcp_* table added by a
...
begin
  ...
  where c.relnamespace = 'plm'::regnamespace
    and c.relkind = 'r'
    and c.relname like 'marvel_dcp\_%';

  if v_tables is null or array_length(v_tables, 1) is null then
    raise exception 'B FAILED: pg_class returned NO plm.marvel_dcp_* tables. The enumeration is '
  ...
  if v_missing is not null then
    raise exception 'B FAILED: expected plm.marvel_dcp_* table(s) absent from pg_class: %',
...
begin
  ...
  if v_tables is null then
    raise exception 'C FAILED: no plm.marvel_dcp_* tables found -- vacuous pass prevented.';
  end if;
...
do $$
begin
  if plm.marvel_dcp_loader_privilege_ok(null, null) is not false then
  ...
  if plm.marvel_dcp_loader_privilege_ok('', '') is not false then
  ...
  if plm.marvel_dcp_loader_privilege_ok('authenticated', 'some_other_user') is not false then
    raise exception 'D FAILED: a non-matching identity did not return FALSE.';
  end if;
  if plm.marvel_dcp_loader_privilege_ok('anon', null) is not false then
  ...
  if plm.marvel_dcp_loader_privilege_ok('service_role', null) is not true then
    raise exception 'D FAILED: service_role did not return TRUE.';
  end if;
  if plm.marvel_dcp_loader_privilege_ok(null, 'postgres') is not true then
...
begin
  ...
  if v_trigs < 10 then
    raise exception 'E FAILED: only % non-internal triggers on plm.marvel_dcp_* tables; expected '
      'at least 10 (4 crawl-scoped + marvel_dcp_load_exception + 3 identity + the crawl freeze + '
  ...
  begin
    foreach v_t in array array[
      ...
      'marvel_dcp_asset_tile_observation','marvel_dcp_load_exception','marvel_dcp_chunk_ledger'
    ] loop
  ...
  insert into plm.marvel_dcp_portal_tile (source_key, first_seen_crawl_id)
  values ('ZZTEST-TILE-A', v_crawl) returning id into v_tile;

  insert into plm.marvel_dcp_style_guide (source_path, folder_name, region, year_segment,
                                   first_seen_crawl_id)
  values ('/zztest/guide/a', 'ZZTEST-GUIDE-A', 'ZZTEST-REGION', 'ZZTEST-YEAR', v_crawl)
  returning id into v_guide;

  insert into plm.marvel_dcp_asset (source_path, style_guide_id, file_name, file_extension,
                             first_seen_crawl_id)
  values ('/zztest/guide/a/zztest-file.zzz', v_guide, 'zztest-file.zzz', 'zzz', v_crawl)
  returning id into v_asset;

  insert into plm.marvel_dcp_crawl_section (crawl_id, portal_tile_id, listing_kind, status,
  ...
  insert into plm.marvel_dcp_asset_tile_observation (crawl_id, marvel_dcp_asset_id, portal_tile_id,
  ...
  -- run that #695/#731 added) died here with `INSERT on plm.marvel_dcp_crawl_gap is refused`.
  --
  ...
  -- Arm the guards.
  update plm.marvel_dcp_crawl set status = 'complete' where crawl_id = v_crawl;

  -- E1. UPDATE of crawl-scoped evidence must be REFUSED.
  v_ok := false;
  begin
    update plm.marvel_dcp_asset_crawl set observed_row_hash = repeat('b', 64)
    where crawl_id = v_crawl;
  exception when sqlstate 'P0001' then v_ok := true;
  end;
  if not v_ok then
    raise exception 'E1 FAILED: an UPDATE of a COMPLETED crawl''s plm.marvel_dcp_asset_crawl row '
  ...
  v_ok := false;
  begin
    delete from plm.marvel_dcp_asset_tile_observation where crawl_id = v_crawl;
  exception when sqlstate 'P0001' then v_ok := true;
  end;
  if not v_ok then
  ...
  begin
    ...
    update plm.marvel_dcp_crawl_gap set attempt_count = 2 where id = v_open_gap;
    if (select attempt_count from plm.marvel_dcp_crawl_gap where id = v_open_gap) <> 2 then
    ...
    v_ok := false;
    begin
      update plm.marvel_dcp_crawl_gap set attempt_count = 99 where id = v_frozen_gap;
    exception when sqlstate 'P0001' then v_ok := true;
    end;
    if not v_ok then
  ...
  v_ok := false;
  begin
    update plm.marvel_dcp_crawl set notes = 'ZZTEST tamper' where crawl_id = v_crawl;
  exception when sqlstate 'P0001' then v_ok := true;
  end;
  if not v_ok then
    raise exception 'E4 FAILED: a COMPLETED plm.marvel_dcp_crawl row was UPDATED.';
  end if;

  v_ok := false;
  begin
    delete from plm.marvel_dcp_crawl where crawl_id = v_crawl;
  exception when sqlstate 'P0001' then v_ok := true;
  end;
  if not v_ok then
    raise exception 'E4 FAILED: a COMPLETED plm.marvel_dcp_crawl row was DELETED -- which would '
  ...
  v_ok := false;
  begin
    insert into plm.marvel_dcp_load_exception (crawl_id, reason_code, reason)
  ...
  -- E7. The marvel_dcp_load_exception carve-out, both halves. A warning recorded BEFORE
  ...
  begin
    ...
    insert into plm.marvel_dcp_load_exception (crawl_id, severity, reason_code, reason)
    values (v_c2, 'warning', 'ZZTEST', 'ZZTEST warning to triage later')
    returning id into v_exc;
    update plm.marvel_dcp_crawl set status = 'complete' where crawl_id = v_c2;

    -- MUST SUCCEED.
    update plm.marvel_dcp_load_exception
      set resolved_at = now(), resolution_note = 'ZZTEST triaged'
      where id = v_exc;
    if (select resolved_at from plm.marvel_dcp_load_exception where id = v_exc) is null then
    ...
    v_ok := false;
    begin
      update plm.marvel_dcp_load_exception set reason = 'ZZTEST rewritten' where id = v_exc;
    exception when sqlstate 'P0001' then v_ok := true;
    end;
    if not v_ok then
...
-- =====================================================================================
-- E8. THE STORED waived_at VALUE. Behaviour of plm.close_marvel_dcp_crawl_gap, not a property of
-- a literal.
--
...
begin
  ...
  perform plm.close_marvel_dcp_crawl_gap(v_gap, 'waived', 'ZZTEST waiver reason', 'ZZTEST-approver',
                                  timestamptz '2026-08-10 23:41:07+00');

  select waived_at into v_at from plm.marvel_dcp_crawl_gap where id = v_gap;
  ...
  raise notice 'E8 PASSED: close_marvel_dcp_crawl_gap stored waived_at at exactly 12:00:00 UTC, '
...
declare
  ...
  v_ok boolean;
begin
  h1 := plm.marvel_dcp_asset_row_hash('s','/p','f','e','r','/g','gid', array['b','a']);
  h2 := plm.marvel_dcp_asset_row_hash('s','/p','f','e','r','/g','gid', array['a','b']);
  if h1 <> h2 then
  ...
  if plm.marvel_dcp_asset_row_hash('s','/p','f','e','r','/g','gid', array['a','b','a']) <> h1 then
  ...
  h3 := plm.marvel_dcp_asset_row_hash('s','/p','f','e', null, '/g','gid', array['a']);
  h4 := plm.marvel_dcp_asset_row_hash('s','/p','f','e', '',   '/g','gid', array['a']);
  if h3 = h4 then
  ...
  h5 := plm.marvel_dcp_asset_row_hash('s','/p','f','e','r','/g','gid', array[]::text[]);
  if h5 = plm.marvel_dcp_asset_row_hash('s','/p','f','e','r','/g','gid', null) then
  ...
  v_ok := false;
  begin
    perform plm.marvel_dcp_asset_row_hash('s','/p' || chr(31) || 'x','f','e','r','/g','gid',
                                   array['a']);
  exception when sqlstate 'P0001' then v_ok := true;
  end;
  if not v_ok then
  ...
  v_ok := false;
  begin
    perform plm.marvel_dcp_asset_row_hash('s','/p','f','e','r','/g','gid',
                                   array['a' || chr(30) || 'b']);
  exception when sqlstate 'P0001' then v_ok := true;
  end;
  if not v_ok then
...
-- would pass whatever plm.close_marvel_dcp_crawl_gap actually stored -- the first version of this
...
-- accepted. plm.close_marvel_dcp_crawl_gap pins waived_at to MIDDAY UTC. Both zones are asserted;
...
begin
  select count(*) into v_n from plm.marvel_dcp_crawl where captured_by like 'ZZTEST%';
  v_total := v_total + v_n;
  select count(*) into v_n from plm.marvel_dcp_portal_tile where source_key like 'ZZTEST%';
  v_total := v_total + v_n;
  select count(*) into v_n from plm.marvel_dcp_style_guide where folder_name like 'ZZTEST%';
  v_total := v_total + v_n;
  select count(*) into v_n from plm.marvel_dcp_asset where file_name like 'zztest%';
  v_total := v_total + v_n;
...
-- =====================================================================================
-- Contract tests for migration 20260811050000 -- the Disney Marvel DCP Vault PHASE 2 metadata
...
--      character; no property-character table exists; plm.marvel_dcp_character has no property
...
begin
  ...
  where p.pronamespace = 'plm'::regnamespace
    and p.proname in (
      'marvel_dcp_metadata_row_hash','marvel_dcp_reject_completed_metadata_change',
      'marvel_dcp_metadata_run_freeze','marvel_dcp_reject_completed_metadata_identity_change'
    );
  if v_fns <> 4 then
  ...
  from pg_proc p where p.pronamespace = 'plm'::regnamespace
    and p.proname = 'marvel_dcp_asset_row_hash';
  if v_fns <> 1 then
    raise exception 'A FAILED: expected exactly ONE plm.marvel_dcp_asset_row_hash, found %. The '
...
-- =====================================================================================
-- B. PRIVILEGES. service_role: SELECT only; every direct write denied. anon/public: nothing.
--    Enumerated from pg_class, NOT from a hand-written list, so a plm.marvel_dcp_metadata_*
  ...
  for r in
    ...
    where c.relnamespace = 'plm'::regnamespace
      ...
      and c.relname in (
        'marvel_dcp_metadata_run','marvel_dcp_metadata_asset','marvel_dcp_property','marvel_dcp_character','marvel_dcp_term',
        'marvel_dcp_asset_property_observation','marvel_dcp_asset_character_observation',
        'marvel_dcp_asset_term_observation','marvel_dcp_metadata_chunk_ledger',
        'marvel_dcp_metadata_load_exception'
      )
  loop
    v_n := v_n + 1;
...
begin
  ...
  where table_schema = 'plm'
    and table_name ~ '^marvel_dcp_(propert.*character|character.*propert)';
  if v_n > 0 then
    raise exception 'D FAILED: % plm property-character table(s) exist. No such table may '
      'ever be created.', v_n;
  end if;

  -- D3. plm.marvel_dcp_character must have NO property column. Its absence is a locked decision,
  -- not an oversight -- a column here is a slot someone eventually fills by pairing the
  -- two arrays on an asset.
  select count(*) into v_n
  from information_schema.columns
  where table_schema = 'plm' and table_name = 'marvel_dcp_character'
    and (column_name like '%propert%');
  if v_n > 0 then
    raise exception 'D FAILED: plm.marvel_dcp_character has % property column(s). Marvel DCP Vault never '
  ...
  if to_regclass('plm.marvel_dcp_asset_property_observation') is null
  or to_regclass('plm.marvel_dcp_asset_character_observation') is null then
...
begin
  ...
  foreach v_t in array array[
    ...
    'marvel_dcp_metadata_chunk_ledger'
  ] loop
    select tg.tgtype into v_ty
    from pg_trigger tg join pg_class c on c.oid = tg.tgrelid
    where c.relnamespace = 'plm'::regnamespace and c.relname = v_t
  ...
  if (select count(*) from plm.marvel_dcp_asset where file_name = 'zztest-shared.zzz') <> 2 then
  ...
  begin
    ...
    begin
      ...
      insert into plm.marvel_dcp_metadata_asset (metadata_run_id, source_crawl_id, marvel_dcp_asset_id)
    ...
    if not v_ok then
      raise exception 'G FAILED: a metadata row named a source crawl its RUN did not '
        'declare. The composite FK to marvel_dcp_metadata_run must forbid this -- otherwise the '
  ...
  if not v_ok then
    raise exception 'G FAILED: metadata was attached to an asset outside the source crawl. '
      'The composite FK to marvel_dcp_asset_crawl must forbid this.';
  end if;
  ...
  v_ok := false;
  begin
    update plm.marvel_dcp_metadata_asset set fetch_status = 'success', http_status = 200,
      raw_metadata = null, retrieved_at = now(),
      source_hash = repeat('a', 64), normalized_hash = repeat('b', 64)
    where metadata_run_id = v_run and marvel_dcp_asset_id = v_asset;
  exception when check_violation then v_ok := true;
  end;
  if not v_ok then
    raise exception 'G FAILED: fetch_status success was accepted with no response body. '
      'A signed-out Marvel DCP Vault session returns HTTP 200 with a tiny zero-record body, so '
  ...
  v_ok := false;
  begin
    update plm.marvel_dcp_metadata_asset set fetch_status = 'failed', failure_code = null
    where metadata_run_id = v_run and marvel_dcp_asset_id = v_asset;
  exception when check_violation then v_ok := true;
  end;
  if not v_ok then
  ...
  v_ok := false;
  begin
    update plm.marvel_dcp_metadata_asset
  ...
  update plm.marvel_dcp_metadata_asset set
  ...
  if (select is_exclusive_raw from plm.marvel_dcp_metadata_asset
      where metadata_run_id = v_run and marvel_dcp_asset_id = v_asset) <> 'ZZTEST-UNKNOWN-SPELLING'
  or (select rights_parse_confident from plm.marvel_dcp_metadata_asset
      where metadata_run_id = v_run and marvel_dcp_asset_id = v_asset) <> false then
  ...
  -- -----------------------------------------------------------------------------------
  insert into plm.marvel_dcp_property (source_system, source_id)
  values ('marvel_dcpvault', 'ZZTEST-PROP-1') returning id into v_prop;
  insert into plm.marvel_dcp_character (source_system, source_id)
  values ('marvel_dcpvault', 'ZZTEST-CHAR-1') returning id into v_char;

  v_ok := false;
  begin
    insert into plm.marvel_dcp_asset_property_observation (metadata_run_id, marvel_dcp_asset_id, marvel_dcp_property_id)
  ...
  begin
    for i in 1 .. 9 loop
      insert into plm.marvel_dcp_property (source_system, source_id)
      values ('marvel_dcpvault', 'ZZTEST-PROP-M-' || i)
      returning id into v_p;
      insert into plm.marvel_dcp_asset_property_observation (metadata_run_id, marvel_dcp_asset_id, marvel_dcp_property_id)
      values (v_run, v_asset, v_p);
    end loop;

    insert into plm.marvel_dcp_asset_character_observation (metadata_run_id, marvel_dcp_asset_id, marvel_dcp_character_id)
    values (v_run, v_asset, v_char);

    select count(*) into v_props from plm.marvel_dcp_asset_property_observation
    where metadata_run_id = v_run and marvel_dcp_asset_id = v_asset;
    select count(*) into v_chars from plm.marvel_dcp_asset_character_observation
    where metadata_run_id = v_run and marvel_dcp_asset_id = v_asset;
    ...
    if exists (
      ...
      where table_schema = 'plm' and table_name ~ '^marvel_dcp_(propert.*character|character.*propert)'
    ) then
  ...
  if (select count(*) from plm.marvel_dcp_metadata_asset
      where metadata_run_id = v_run and marvel_dcp_asset_id = v_asset) <> 1 then
  ...
  where metadata_run_id = v_run and marvel_dcp_asset_id = v_asset2;

  if (select count(*) from plm.marvel_dcp_asset_character_observation
      where metadata_run_id = v_run and marvel_dcp_asset_id = v_asset2) <> 0 then
    raise exception 'E FAILED: asset2 should have zero character links.';
  end if;
  if (select fetch_status from plm.marvel_dcp_metadata_asset
      where metadata_run_id = v_run and marvel_dcp_asset_id = v_asset2) <> 'success' then
  ...
  -- -----------------------------------------------------------------------------------
  update plm.marvel_dcp_metadata_run set
  ...
  v_ok := false;
  begin
    update plm.marvel_dcp_metadata_asset set dc_title = 'ZZTEST-CHANGED'
    where metadata_run_id = v_run and marvel_dcp_asset_id = v_asset;
  exception when sqlstate 'P0001' then v_ok := true;
  end;
  if not v_ok then
  ...
  v_ok := false;
  begin
    delete from plm.marvel_dcp_asset_property_observation
    where metadata_run_id = v_run and marvel_dcp_asset_id = v_asset;
  exception when sqlstate 'P0001' then v_ok := true;
  end;
  if not v_ok then
  ...
  v_ok := false;
  begin
    update plm.marvel_dcp_metadata_run set crawler_version = 'ZZTEST-9'
    where metadata_run_id = v_run;
  exception when sqlstate 'P0001' then v_ok := true;
  end;
  if not v_ok then
  ...
  v_ok := false;
  begin
    delete from plm.marvel_dcp_metadata_run where metadata_run_id = v_run;
  exception when sqlstate 'P0001' then v_ok := true;
  end;
  if not v_ok then
  ...
  update plm.marvel_dcp_character set last_seen_metadata_run_id = v_run2 where id = v_char;
...
begin
  ...
  v_a := plm.marvel_dcp_metadata_row_hash(
  ...
  v_b := plm.marvel_dcp_metadata_row_hash(
    'ZZTEST-U', null, null, null, null, null, null, null, null, null, null, null,
    null, null, null, null, null, null,
    array['ZZTEST-P2','ZZTEST-P1'], array['ZZTEST-C1'], array[]::text[], null);
  v_c := plm.marvel_dcp_metadata_row_hash(
    'ZZTEST-U', null, null, null, null, null, null, null, null, null, null, null,
    null, null, null, null, null, null,
  ...
  v_d := plm.marvel_dcp_metadata_row_hash(
    'ZZTEST-U', null, null, null, null, null, null, null, null, null, null, null,
    null, null, null, null, null, null,
  ...
  begin
    begin
      perform plm.marvel_dcp_metadata_row_hash(
  ...
  if (select pronargs from pg_proc
      where pronamespace = 'plm'::regnamespace and proname = 'marvel_dcp_metadata_row_hash') <> 22 then
    raise exception 'F FAILED: plm.marvel_dcp_metadata_row_hash does not take exactly 22 '
...
begin
  select count(*) into v_n from plm.marvel_dcp_crawl where captured_by like 'ZZTEST%';
  v_total := v_total + v_n;
  select count(*) into v_n from plm.marvel_dcp_metadata_run where captured_by like 'ZZTEST%';
  v_total := v_total + v_n;
  select count(*) into v_n from plm.marvel_dcp_property where source_id like 'ZZTEST%';
  v_total := v_total + v_n;
  select count(*) into v_n from plm.marvel_dcp_character where source_id like 'ZZTEST%';
  v_total := v_total + v_n;
  select count(*) into v_n from plm.marvel_dcp_term where source_value like 'ZZTEST%';
  v_total := v_total + v_n;
  select count(*) into v_n from plm.marvel_dcp_asset where file_name like 'zztest%';
  v_total := v_total + v_n;
...
-- =====================================================================================
-- Contract tests for migration 20260811060000 -- the Disney Marvel DCP Vault PHASE 2 metadata
...
--   plm.marvel_dcp_loader_privilege_ok, which is TRUE only for JWT role service_role or
...
declare
  ...
  v_bad text;
begin
  if not plm.marvel_dcp_loader_privilege_ok(auth.role(), session_user) then
    raise exception 'A FAILED: this connection does not satisfy '
      'plm.marvel_dcp_loader_privilege_ok (JWT role %L / session_user %L). The loader contract '
  ...
  if to_regclass('plm.marvel_dcp_metadata_chunk_ledger') is null
  or to_regclass('plm.marvel_dcp_metadata_load_exception') is null then
    raise exception 'A FAILED: a loader table is missing.';
  end if;
  if has_table_privilege('service_role', 'plm.marvel_dcp_metadata_chunk_ledger', 'TRUNCATE')
  or has_table_privilege('service_role', 'plm.marvel_dcp_metadata_load_exception', 'TRUNCATE') then
...
begin
  ...
  -- ===================================================================================
  -- B. begin_marvel_dcp_metadata_run
  -- ===================================================================================
  ...
  v_ok := false;
  begin
    perform plm.begin_marvel_dcp_metadata_run('marvel_dcpvault', v_partial, date '2026-02-04', '/zztest/metadata',
  ...
  v_ok := false;
  begin
    perform plm.begin_marvel_dcp_metadata_run('marvel_dcpvault', v_empty, date '2026-02-04', '/zztest/metadata',
      'ZZTEST-0', 'ZZTEST-runner', repeat('a', 40));
  exception when sqlstate 'P0001' then v_ok := true;
  end;
  if not v_ok then
  ...
  v_run := plm.begin_marvel_dcp_metadata_run('marvel_dcpvault', v_crawl, date '2026-02-04', '/zztest/metadata',
    'ZZTEST-0', 'ZZTEST-runner', repeat('a', 40));

  if (select assets_expected from plm.marvel_dcp_metadata_run where metadata_run_id = v_run) <> 2 then
    raise exception 'B FAILED: assets_expected was not counted from plm.marvel_dcp_asset_crawl.';
  end if;
  select count(*) into v_n from plm.marvel_dcp_metadata_asset
  ...
  v_ok := false;
  begin
    perform plm.begin_marvel_dcp_metadata_run('marvel_dcpvault', v_crawl, date '2026-02-05', '/zztest/metadata',
      'ZZTEST-0', 'ZZTEST-runner', repeat('a', 40));
  exception when sqlstate 'P0001' then v_ok := true;
  end;
  if not v_ok then
  ...
  v_ok := false;
  begin
    perform plm.load_marvel_dcp_metadata_chunk('marvel_dcpvault', v_run, 1, v_json, repeat('0', 64));
  exception when sqlstate 'P0001' then v_ok := true;
  end;
  if not v_ok then
    raise exception 'C FAILED: a chunk with a wrong digest was accepted.';
  end if;
  if (select count(*) from plm.marvel_dcp_metadata_chunk_ledger where metadata_run_id = v_run) <> 0 then
  ...
  v_ok := false;
  begin
    perform plm.load_marvel_dcp_metadata_chunk('marvel_dcpvault', v_run, 1, v_json, v_sha);
  exception when sqlstate 'P0001' then v_ok := true;
  end;
  if not v_ok then
  ...
  v_json :=
    ...
    || '"keywords":["ZZTEST-K1"]}]';
  v_sha := encode(sha256(convert_to(v_json, 'UTF8')), 'hex');
  v_res := plm.load_marvel_dcp_metadata_chunk('marvel_dcpvault', v_run, 1, v_json, v_sha);
  ...
  select count(*) into v_n from plm.marvel_dcp_asset_property_observation
  where metadata_run_id = v_run and marvel_dcp_asset_id = v_a1;
  if v_n <> 9 then
    raise exception 'D FAILED: expected 9 property links after a 10-element array with one '
      'duplicate, got %.', v_n;
  end if;
  select count(*) into v_n from plm.marvel_dcp_asset_character_observation
  where metadata_run_id = v_run and marvel_dcp_asset_id = v_a1;
  if v_n <> 1 then
  ...
  if exists (
    ...
    where c.table_schema = 'plm' and c.column_name in ('marvel_dcp_property_id','marvel_dcp_character_id')
  ...
  select count(*) into v_n
  from plm.marvel_dcp_asset_term_observation o join plm.marvel_dcp_term t on t.id = o.marvel_dcp_term_id
  where o.metadata_run_id = v_run and o.marvel_dcp_asset_id = v_a1 and t.term_kind = 'art_style';
  if v_n <> 0 then
    raise exception 'E FAILED: an empty art_styles array produced % link(s).', v_n;
  end if;
  select count(*) into v_n
  from plm.marvel_dcp_asset_term_observation o join plm.marvel_dcp_term t on t.id = o.marvel_dcp_term_id
  where o.metadata_run_id = v_run and o.marvel_dcp_asset_id = v_a1 and t.term_kind = 'keyword';
  if v_n <> 1 then
    raise exception 'E FAILED: expected 1 keyword link, got %.', v_n;
  end if;
  if (select fetch_status from plm.marvel_dcp_metadata_asset
      where metadata_run_id = v_run and marvel_dcp_asset_id = v_a1) <> 'success' then
  ...
  from plm.marvel_dcp_metadata_asset where metadata_run_id = v_run and marvel_dcp_asset_id = v_a1;
  ...
  select plm.marvel_dcp_metadata_row_hash(
    ...
    (select coalesce(array_agg(p.source_id), array[]::text[])
       from plm.marvel_dcp_asset_property_observation o join plm.marvel_dcp_property p on p.id = o.marvel_dcp_property_id
      where o.metadata_run_id = v_run and o.marvel_dcp_asset_id = v_a1),
    (select coalesce(array_agg(c.source_id), array[]::text[])
       from plm.marvel_dcp_asset_character_observation o join plm.marvel_dcp_character c on c.id = o.marvel_dcp_character_id
      where o.metadata_run_id = v_run and o.marvel_dcp_asset_id = v_a1),
  ...
  where m.metadata_run_id = v_run and m.marvel_dcp_asset_id = v_a1;
...
declare
  ...
  v_props int;
begin
  select metadata_run_id into v_run from plm.marvel_dcp_metadata_run
  where source_crawl_id = v_crawl and status = 'running';
  select id into v_a1 from plm.marvel_dcp_asset where source_path = '/zztest/lguide/one.zzz';
  select id into v_a2 from plm.marvel_dcp_asset where source_path = '/zztest/lguide/two.zzz';
  select id into v_a3 from plm.marvel_dcp_asset where source_path = '/zztest/lguide/three.zzz';
  ...
  v_sha := encode(sha256(convert_to(v_json, 'UTF8')), 'hex');
  v_res := plm.load_marvel_dcp_metadata_chunk('marvel_dcpvault', v_run, 2, v_json, v_sha);
  ...
  if not exists (
    select 1 from plm.marvel_dcp_metadata_load_exception
  ...
  if exists (select 1 from plm.marvel_dcp_metadata_asset
             where metadata_run_id = v_run and marvel_dcp_asset_id = v_a3) then
  ...
  if not exists (
    select 1 from plm.marvel_dcp_metadata_chunk_ledger
    where metadata_run_id = v_run and chunk_number = 2
  ...
  v_ok := false;
  begin
    perform plm.finalize_marvel_dcp_metadata_run(v_run);
  exception when sqlstate 'P0001' then v_ok := true;
  end;
  if not v_ok then
  ...
  v_sha := encode(sha256(convert_to(v_json, 'UTF8')), 'hex');
  v_res := plm.load_marvel_dcp_metadata_chunk('marvel_dcpvault', v_run, 3, v_json, v_sha);
  ...
  update plm.marvel_dcp_metadata_load_exception
    set resolved_at = now(), resolution_note = 'ZZTEST triaged'
  ...
  v_res := plm.load_marvel_dcp_metadata_chunk('marvel_dcpvault', v_run, 9, v_json, v_sha);
  ...
  -- ---------------------------------------------------------------------------------
  -- G4. fail_marvel_dcp_metadata_run PRESERVES EVERYTHING and requires a reason.
  -- ---------------------------------------------------------------------------------
  v_ok := false;
  begin
    perform plm.fail_marvel_dcp_metadata_run(v_run, '');
  exception when sqlstate 'P0001' then v_ok := true;
  end;
  if not v_ok then
  ...
  select count(*) into v_props from plm.marvel_dcp_asset_property_observation
  where metadata_run_id = v_run;
  perform plm.fail_marvel_dcp_metadata_run(v_run, 'ZZTEST deliberate failure');

  if (select status from plm.marvel_dcp_metadata_run where metadata_run_id = v_run) <> 'failed' then
    raise exception 'G FAILED: the run did not reach status failed.';
  end if;
  select count(*) into v_n from plm.marvel_dcp_asset_property_observation where metadata_run_id = v_run;
  if v_n <> v_props then
  ...
  v_ok := false;
  begin
    update plm.marvel_dcp_metadata_run set status = 'running' where metadata_run_id = v_run;
  exception when sqlstate 'P0001' then v_ok := true;
  end;
  if not v_ok then
  ...
  v_ok := false;
  begin
    perform plm.load_marvel_dcp_metadata_chunk('marvel_dcpvault', v_run, 10, v_json, v_sha);
  exception when sqlstate 'P0001' then v_ok := true;
  end;
  if not v_ok then
...
declare
  ...
  v_res   jsonb;
begin
  v_run := plm.begin_marvel_dcp_metadata_run('marvel_dcpvault', v_crawl, date '2026-02-09', '/zztest/metadata',
  ...
  v_res := plm.finalize_marvel_dcp_metadata_run(v_run);
  ...
  if (select count(*) from plm.marvel_dcp_asset_character_observation where metadata_run_id = v_run) <> 0 then
  ...
  begin
    select id into v_c from plm.marvel_dcp_character limit 1;
    begin
      insert into plm.marvel_dcp_asset_character_observation (metadata_run_id, marvel_dcp_asset_id, marvel_dcp_character_id)
      select v_run, m.marvel_dcp_asset_id, v_c from plm.marvel_dcp_metadata_asset m
...
begin
  ...
  select count(*) into v_n from plm.marvel_dcp_term where source_value like 'ZZTEST%';
  v_total := v_total + v_n;
  select count(*) into v_n from plm.marvel_dcp_metadata_load_exception where reason_code like 'ZZTEST%';
  v_total := v_total + v_n;
