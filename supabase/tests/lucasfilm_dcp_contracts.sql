...
begin
  ...
  where c.relnamespace = 'plm'::regnamespace
    and c.relkind = 'r'
    and c.relname like 'lucasfilm_dcp\_%';
...
begin
  ...
  perform plm.set_source_resolution(
    'lucasfilm_dcpvault','style_guide',
    (select 'path:' || source_path from plm.lucasfilm_dcp_style_guide where id=v_guide),
...
begin
  ...
  perform plm.set_source_resolution(
    'lucasfilm_dcpvault','character',
    (select 'id:' || source_id from plm.lucasfilm_dcp_character where id=v_char),
    'deferred',null,null,null,null,'ZZTEST reviewed',null
  );
  if not exists (
    select 1 from plm.source_resolution
    where source_system='lucasfilm_dcpvault' and entity_kind='character'
      and source_id=(select 'id:' || source_id from plm.lucasfilm_dcp_character where id=v_char)
