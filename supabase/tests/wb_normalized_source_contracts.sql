...
-- Issue #995. The only truthful empty stream is direct Franchise-to-Property
-- evidence. Blank optional Asset values land as SQL NULL; identity stays strict.
begin;
do $truthful_empty_and_nulls$
declare
  empty_hash text:=encode(extensions.digest(convert_to('','UTF8'),'sha256'),'hex');
  empty_capture uuid;
  asset_capture uuid:='99999999-9999-4999-8999-000000000995';
  asset_snapshot jsonb;
  r record;
  rejected boolean;
begin
  empty_capture:=plm.begin_wb_capture(
    'wb_franchise_property_evidence', date '2099-01-10', 'synthetic-empty',
    empty_hash, 0, 'synthetic', 'https://example.invalid', null
  );
  select * into r from plm.finalize_wb_capture(empty_capture,empty_hash,1);
  if r.rows_seen<>0 or r.rows_landed<>0 or r.rows_inserted<>0 or r.rows_updated<>0 then
    raise exception 'truthful zero-row evidence capture reported invented rows';
  end if;
  if not exists(
    select 1 from plm.wb_capture
    where capture_id=empty_capture and chunk_number=0 and status='complete'
  ) then raise exception 'truthful zero-row evidence capture did not complete'; end if;

  rejected:=false;
  begin
    perform plm.begin_wb_capture(
      'wb_asset_normalized', date '2099-01-10', 'synthetic-empty-asset',
      empty_hash, 0, 'synthetic', 'https://example.invalid', null
    );
  exception when sqlstate 'P0001' then rejected:=true; end;
  if not rejected then raise exception 'zero-row required Asset stream unexpectedly began'; end if;
  ...
  insert into plm.wb_capture(
    ...
    asset_capture,0,'wb_asset_normalized','validating',date '2099-01-10',
    'synthetic-nullables',repeat('d',64),1,'synthetic','https://example.invalid',now()
  );
  asset_snapshot:=jsonb_build_object(
    'captured_at','2099-01-10','rows',jsonb_build_array(jsonb_build_object(
      'source_namespace','synthetic','source_id','nullable-asset',
      'warner_asset_id','','file_name','synthetic.bin','source_path','synthetic/path',
      'season','','file_size_bytes','','source_created_at','','source_modified_at','',
      'source_url','https://example.invalid'
    ))
  );
  select * into r from plm.sync_wb_normalized_target(asset_capture,'wb_asset_normalized',asset_snapshot,'mirror_only',1);
  if r.rows_inserted<>1 then raise exception 'nullable synthetic Asset was not inserted'; end if;
  if not exists(
    select 1 from plm.wb_asset_normalized
    where source_namespace='synthetic' and source_id='nullable-asset'
      and warner_asset_id is null and season is null and file_size_bytes is null
      and source_created_at is null and source_modified_at is null
  ) then raise exception 'blank optional Asset values did not normalize to SQL NULL'; end if;

  rejected:=false;
  begin
    perform plm.wb_validate_normalized_row('wb_asset_normalized',jsonb_build_object(
      'source_namespace','synthetic','source_id','','file_name','synthetic.bin',
      'source_path','synthetic/path','source_url','https://example.invalid'
    ));
  exception when sqlstate 'P0001' then rejected:=true; end;
  if not rejected then raise exception 'blank required Asset identity unexpectedly passed'; end if;
end
$truthful_empty_and_nulls$;
rollback;
