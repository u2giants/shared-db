...
begin
  ...
  foreach t in array v_tables loop
    ...
    --
    -- twentieth_century_dcp_metadata_chunk_ledger (migration 20260811060000, the Phase-2 loader) was added
    -- to this carve-out because this section enumerates plm.twentieth_century_dcp_* FROM pg_class rather
    ...
    if t not in ('twentieth_century_dcp_chunk_ledger', 'twentieth_century_dcp_metadata_chunk_ledger')
...
begin
  ...
  update plm.twentieth_century_dcp_style_guide set resolution_reason = 'ZZTEST reviewed' where id = v_guide;
...
begin
  ...
  update plm.twentieth_century_dcp_character set resolution_note = 'ZZTEST reviewed', resolved_at = now()
  where id = v_char;
  if (select resolution_note from plm.twentieth_century_dcp_character where id = v_char) <> 'ZZTEST reviewed' then
